...
# postgres-init, proxy-templates, README, e i .env.example.
#
# Cosa NON tocca mai: i file d'ambiente veri (release.env, fidelity.env,
# olimpo.env), i certificati, la configurazione del cliente in fidelity-config,
# le directory di dati. Appartengono alla
...
if [ "$bundle" = "$destinazione" ]; then
    ...
    rosso "  cd /opt/company-ai/projects/<progetto> && sudo /tmp/fidelity-olimpo/install-bundle.sh"
    exit 1
fi
...
copia dump-databases.sh
copia seed-fidelity.sh
copia postgres-init
copia proxy-templates
copia README.md
copia release.env.example
copia fidelity.env.example
copia olimpo.env.example

chmod +x "$destinazione/prepare-target.sh" "$destinazione/install-bundle.sh"     "$destinazione/dump-databases.sh" "$destinazione/seed-fidelity.sh" 2>/dev/null

echo
echo "File d'ambiente"
ambiente release.env.example  release.env
ambiente fidelity.env.example fidelity.env
ambiente olimpo.env.example   olimpo.env
...
echo "    up -d --force-recreate fidelity olimpo"
echo
