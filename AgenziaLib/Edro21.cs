...
namespace AgenziaLib
{
    ...
    internal class Edro21 : IAgenzia
    {
        ...
        //public string esportaVolantinoOld(Dictionary<string, string> formRequest, List<Dictionary<string, object>> tracciato, string pathMeccaniche, string pathOrdinamentoLista, string pathAree, string pathMastro)
        //{
        //    string potenziale_esempio = Edro21Context.Meta.potenziale_esempio;
        //    string meccanica_origine = Edro21Context.Meta.meccanica_origine;
        //    string keyArea = Edro21Context.Meta.area;
        ...
        //    string referenza_pilota = Edro21Context.Meta.referenza_pilota;
        //    result.liste = new List<Tracciato>() { tracciato_da_esportare };
        //    result.liste[0].NomeEsportazione = formRequest["Titolo"];
        //    result.liste[0].Records = new List<Dictionary<string, object>>();
        //    try
        //    {
        //        requestParams = formRequest;
        ...
        //        if (formRequest["importFieldsStringfy"].Length > 1 && formRequest["importFieldsStringfy"][formRequest["importFieldsStringfy"].Length - 2] == '&')
        //        {
        //            formRequest["importFieldsStringfy"] = formRequest["importFieldsStringfy"].Remove(formRequest["importFieldsStringfy"].Length - 2, 1);
        //        }

        //        string queryString = formRequest["importFieldsStringfy"];
        ...
        //        //Dictionary<string, string> reqImport = nameValueCollection.AllKeys
        ...
        //        string tipo_materiale = "vol";// reqImport["cmbMaterialeVol"];
        //        Byte tipo_volantino = 1;// Byte.Parse(reqImport["cmbTipoTracciato"]);

        //        Dictionary<string, object> reqTracciato = JsonConvert.DeserializeObject<Dictionary<string, object>>(formRequest["tracciatoFieldsStringfy"]);

        //        //queryString = formRequest["tracciatoFieldsStringfy"];

        //        //// Usa HttpUtility.ParseQueryString per convertire la stringa in un NameValueCollection
        //        //nameValueCollection = HttpUtility.ParseQueryString(queryString);

        //        //// Converte il NameValueCollection in un dizionario
        //        //Dictionary<string, string> reqTracciato = nameValueCollection.AllKeys
        //        //    .ToDictionary(key => key, key => nameValueCollection[key]);
        ...
        //        //bool esportazione_da_menabo = (formRequest["esportaDaMenabo"] == "true");
        ...
        //        for (int i = 0; i < tracciato.Count; i++)
        //        {
        //            Dictionary<string, object> recItemGroup = tracciato[i];
        ...
        //                    //Apertura record
        //                    //recItem["meccanica_tradotta"] = recItem[key_combinazioneAssegnata];
        ...
        //                            recItem[key_combinazioneAssegnata].ToString().Contains("50al50Fid") ||
        //                            meccanica == "PUNTI" ||
        //                            meccanica == "PUNTI + %" ||
        ...
        //                    try
        //                    {
        //                        if (meccanica == "TP FID" && recItem[key_combinazioneAssegnata].ToString().IndexOf("PUNTI_TP") >= 0)
        //                        {
        ...
        //                    if (differenza_etto_note != "" && recItem[key_combinazioneAssegnata].ToString().Contains("_boxetto"))
        //                    {
        ...
        //            {
        //                recItemGroup[keyRefs] = myGroupNew;
        //                result.liste[0].Records.Add(recItemGroup);
        //            }
...
Descrizione3.EndsWith("\r\n");
                        ...
                        if (_codSottoGruppo != "" && _codSottoGruppo != recItem[GLOBAL_VARIABLES.keyRefCodice] && !inCache)
                        {
                            ...
                            if (elementiSottogruppo.Count > 1)
                            {
                                ...
                                try
                                {
                                    ...
                                    //            var nota_esempio = recItem["nota_esempio"].ToString().ToLower();

                                    //            string descrizioneEsempio = listExtra["nota_esempio"].content;
                                    //            if (descrizioneEsempio != null && descrizioneEsempio != "" &&
                                    //                codice_gruppo != codice_referenza &&
                                    //                (byte)recItem["StatoSelezione"] == (byte)1 &&
                                    //                nota_esempio.Contains("esempio") && !nota_esempio.Contains("EVIDENZIATO ESEMPIO MA NON NECESSARIO".ToLower()))
                                    //            {
                                    //                desEsempio = true;
                                    //                //DA AGGIORNARE IN MAPPA STILI
                                    ...
                                    //                descrizione += "<" + nome_stile_esempio + ">" + unEsempio + descrizioneEsempio + "</" + nome_stile_esempio + ">";
                                    //            }
                                    //        }
                            ...
                            else
                            {
                                ...
                                if (id_mastro > 0)
                                {
                                    ...
                                    if (foto || esempio || tipo_volantino == 2)// || req.confronta_liste)
                                    {
                                        ...
                                        {
                                            /*
                                            string nome_materiale = liste_da_esportare[l].combinazione.IdMaterialeNavigation.Nome;
                                            string formato = liste_da_esportare[l].combinazione.IdFormatoNavigation.Nome;
                                            if (nome_materiale == "CONAD_CARD")
                                            {
                                                if (formato == "A4" || formato == "stopper")
                                                    nome_mastro = "A-Mastro_001";
                                                else if (formato == "70x100")
                                                    nome_mastro = "A-Mastro_002";
                                                else if (formato == "95x45")
                                                    nome_mastro = "A-Mastro_012";
                                            }
                                            else if (formato == "70x100")
                                            {
                                                if (nome_materiale == "REMART" || nome_materiale == "REGIOV")
                                                {
                                                    nome_mastro = "A-Mastro_001";
                                                }
                                                else if (nome_materiale == "FeC")
                                                {
                                                    nome_mastro = "A-Mastro_040";
                                                }
                                                else
                                                {
                                                    nome_mastro = "A-Mastro_024";
                                                }

                                            }
                                            else if (formato == "50x70")
                                            {
                                                nome_mastro = "A-Mastro_050";
                                            }
                                            */
                                        }
        ...
        protected string[] get4Descrizioni(Dictionary<string, object> tItem, bool is_pop, bool esporta_da_archivio, bool confronto_liste)//, PopCombinazioni popComb = null)
        {
            ...
            try
            {
                ...
                /*
                //Controllo tipologia per la casistica particolare di POP Reparto 33
                if (is_pop &&  reparto == 33 && unitaFatt == "peso")
                {
                    
                    string formato_pop = popComb.IdFormatoNavigation.Nome;
                    string tipomat = popComb.IdMaterialeNavigation.Nome;
                    if (formato_pop == "stopper" || formato_pop == "A3" || formato_pop == "A4" ||
                        (formato_pop == "70x100" && (tipomat == "REMART" || tipomat == "REGIOV")))
                    {
                        string new_tipo = result[1];
                        if (new_tipo == "")
                        {
                            //Caso 1
                            new_tipo = String.Format("{0}{1}{2}{3}{4}", "Origine", Environment.NewLine, "categoria", Environment.NewLine, "calibro");
                        }
                        else
                        {
                            string str_for_search = new_tipo.ToLower();
                            if (!str_for_search.Contains("origine") &&
                                !str_for_search.Contains("categoria") &&
                                    !str_for_search.Contains("calibro"))
                            {
                                //caso 3
                                new_tipo = String.Format("{0}{1}{2}{3}{4}{5}{6}", new_tipo, Environment.NewLine, "Origine", Environment.NewLine, "categoria", Environment.NewLine, "calibro");
                            }
                            else
                            {
                                int inx_origine = str_for_search.IndexOf("origine");
                                int inx_cat = str_for_search.IndexOf("categoria");
                                int inx_calibro = str_for_search.IndexOf("calibro");
                                bool intatta = (inx_origine >= 0 && inx_cat >= 0 && inx_calibro >= 0);
                                ...
                                if (!intatta)
                                {
                                    ...
                                    if (inx_origine >= 0)
                                    {
                                        str_origine = new_tipo.Substring(inx_origine);
                                        if (inx_origine > 0)
                                            altro = new_tipo.Substring(0, inx_origine);
                                    }
                                    else
                                    {
                                        str_origine = "Origine";
                                    ...
                                    {
                                        if (inx_origine >= 0)
                                            str_origine = new_tipo.Substring(inx_origine, inx_cat - inx_origine);

                                        str_cat = new_tipo.Substring(inx_cat);
                                        str_cat = str_cat[0].ToString().ToLower() + str_cat.Substring(1);

                                        if (inx_origine < 0 && inx_cat > 0)
                                            altro = new_tipo.Substring(0, inx_cat);
                                    }
                                    else
                                    {
                                        str_cat = "categoria";
                                    ...
                                    {
                                        if (inx_origine >= 0 && inx_cat < 0)
                                            str_origine = new_tipo.Substring(inx_origine, inx_calibro - inx_origine);

                                        if (inx_cat >= 0)
                                        {
                                            str_cat = new_tipo.Substring(inx_cat, inx_calibro - inx_cat);
                                            str_cat = str_cat[0].ToString().ToLower() + str_cat.Substring(1);
                                        }

                                        str_calibro = new_tipo.Substring(inx_calibro).ToLower();
                                        str_calibro = str_calibro[0].ToString().ToLower() + str_calibro.Substring(1);


                                        if (inx_origine < 0 && inx_cat < 0 && inx_calibro > 0)
                                            altro = new_tipo.Substring(0, inx_calibro);
                                    }
                                    else
                                    {
                                        str_calibro = "calibro";
                                    ...
                                    if (str_origine.LastIndexOf(Environment.NewLine) > 0 && str_origine.LastIndexOf(Environment.NewLine) >= str_origine.Length - 2)
                                        str_origine = str_origine.Substring(0, str_origine.Length - 2);
                                    if (str_cat.LastIndexOf(Environment.NewLine) > 0 && str_cat.LastIndexOf(Environment.NewLine) >= str_cat.Length - 2)
                                        str_cat = str_cat.Substring(0, str_cat.Length - 2);
                                    if (altro.LastIndexOf(Environment.NewLine) > 0 && altro.LastIndexOf(Environment.NewLine) >= altro.Length - 2)
                                        altro = altro.Substring(0, altro.Length - 2);

                                    new_tipo = "";
                                    if (altro != "")
                                        new_tipo += altro;
                                    if (str_origine != "")
                                        new_tipo += (new_tipo != "" ? Environment.NewLine : "") + str_origine.Replace(",", "");//.Replace(Environment.NewLine, " ").Replace("  ", " ");
                                    if (str_cat != "")
                                        new_tipo += (new_tipo != "" ? Environment.NewLine : "") + str_cat.Replace(",", "");//.Replace(Environment.NewLine, " ").Replace("  ", " ");
                                    if (str_calibro != "")
                                        new_tipo += (new_tipo != "" ? Environment.NewLine : "") + str_calibro.Replace(",", "");//.Replace(Environment.NewLine, " ").Replace("  ", " ");
                                }
    ...
    List<Dictionary<string, object>> myGroup)
        ...
        protected IndiceDifferenza getIndiceDifferenze(Dictionary<string, object> item_esempio, Dictionary<string, object> current, string areaTracciato = "")
        {
            ...
            if (gItem != null)
            {
                ...
                //    tipo_volantino = Byte.Parse(reqImport["cmbTipoTracciato"]);
                //    id_area = Int32.Parse(reqImport["cmbCanaleArea"]);
                //}
                ...
                //    id_area = Int32.Parse(requestParams["cmbCanaleArea"]);
                //    if (areaTracciato!="" && dbAree!=null)
                //    {
                //        AreaItem aItem = dbAree.Where(ar => ar.Area == areaTracciato).FirstOrDefault();
                //        if(aItem!=null)
                //        {
                //            id_area = aItem.Id;
                //        }
        ...
        protected List<Dictionary<string, string>> getLoghiEBolli(Dictionary<string, object> oItem, string[] descrizioni, Ordinamento gItem)
        {
            ...
            //                prodotto_conad = "aslattosio";
            //            else
            //                prodotto_conad = "asglutine";
            //            //else
            //            //prodotto_conad = "logo";
            //        }
            ...
            //    if (gusto.Contains("selezione conad"))
            //        selezione_conad = "True";

            //    if (/*(nota_category.Contains("tv") && nota_category.Contains("visto")) ||*/
            //        (tema.Contains("tv") && tema.Contains("vist")) ||
            //        (sezione.Contains("tv") && sezione.Contains("vist")) ||
            //        (tipo_tema.Contains("tv") && tipo_tema.Contains("vist")) ||
            //        ruolo.Contains("tv") ||
            //        tema.Contains("cedipiu")
            //        )
            //    {
            ...
            //    if (brand.Contains("conad")/* && gItem.food*/)
            //    {
            ...
            //            {
            //                string[] analisi_soggetto2 = s.Split(new string[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries);
            ...
            //        if (logo_carne != "")
            //            logo_carne += ",";
            //        logo_carne += "LOGO_Carne_Chianina";
            //    }
            ...
            //    if (nota_category.Contains("pront") && nota_category.Contains("mangiare") && reparto == 31)
            //    {
            //        logo_pesce = "Logo_PE_PRONTIdaMangiare";
            //    }
            //    if (nota_category.Contains("pront") && nota_category.Contains("cuocere") && reparto == 31)
            //    {
            //        if (logo_pesce != "")
            //            logo_pesce += ",";
            //        logo_pesce += "Logo_PE_PRONTIdaCuocere";
            //    }
            //    if (nota_category.Contains("conf") && nota_category.Contains("naturale") && reparto == 31)
            //    {
            //        if (logo_pesce != "")
            //            logo_pesce += ",";
            //        logo_pesce += "Logo_PE_CONFalNaturale";
            //    }
            //    if (nota_category.Contains("sea") && nota_category.Contains("friend"))
            //    {
            //        if (logo_pesce != "")
            //            logo_pesce += ",";
            //        logo_pesce += "Logo_Friend-of-the-Sea";
            //    }
            //    if (nota_category.Contains("marinou"))
            //    {
            //        if (logo_pesce != "")
            //            logo_pesce += ",";
    ...
    (nota_category.Contains("bass") && nota_category.Contains("fiss"));
        ...
        //    sw1.Start();
        //    foreach (var item in tracciatoSingoli)
        //    {
        ...
        //        {
        //            var elementoCorrispondenteAC = listaOrigine.Find(f => f.Area == ac.area && f.Canale == ac.canale && f.Dato[Meta.ReferenzaCodice] == item.Dato[Meta.ReferenzaCodice]);

        //            if(elementoCorrispondenteAC == null)
        //            {
        //                Debug.WriteLine("Errore");
        //                continue;
        //            }

        //            ac.InVol = (elementoCorrispondenteAC.Dato[Meta.tipo_volantino] as string).Contains("V - Volantino");
        //        } 
        //        sw.Stop();
        //        sw.ToString();
        //    }
        ...
        //public List<q_records_per_getListaRevisione> specificaInOutVol(List<q_records_per_getListaRevisione> tracciatoRevisione, List<q_records_per_getListaRevisione> listaOrigine)
        //{
        //    Stopwatch sw1 = new Stopwatch();
        //    sw1.Start();
        //    // Crea un dizionario chiave composta: Area + Canale + ReferenzaCodice
        //    var lookup = listaOrigine.ToDictionary(
        //        f => $"{f.Area}_{f.Canale}_{f.Dato[Meta.ReferenzaCodice]}_{f.Label}",
        //        f => f
        //    );

        //    foreach (var item in tracciatoRevisione)
        //    {
        //        var acComuni = item.Dato[Meta.keyACComuni] as List<ACcomuni>;
        //        if (acComuni == null) continue; // Evitiamo NullReferenceException

        //        foreach (var ac in acComuni)
        //        {
        //            var key = $"{ac.area}_{ac.canale}_{item.Dato[Meta.ReferenzaCodice]}_{ac.label}";

        //            if (!lookup.TryGetValue(key, out var elementoCorrispondenteAC))
        //            {
        //                Debug.WriteLine("Errore");
        //                continue;
        //            }
    ...
    string siglaTracciato)
        ...
        //public bool CheckFirmaGarantita(List<Dictionary<string, object>> recordsGruppo, string meta)
        //{
        //    if (recordsGruppo == null || recordsGruppo.Count == 0)
        //    {
        //        return false;
        ...
        //    DateTime dataEstrazioneRecord;
        //    DateTime dataEstrazioneMeta;

        //    try
        //    {
        //        var struttura = JsonConvert.DeserializeObject<StrutturaMetaPerRevisione>(meta);

        //        if (struttura == null || string.IsNullOrWhiteSpace(struttura.chiaviMeta))
        //        {
        //            return false;
        //        }
        ...
        //    return dataEstrazioneMeta >= dataEstrazioneRecord;
        //}
