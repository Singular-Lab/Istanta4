...
namespace Istanta.Controllers
{
    ...
    public class MeccanicheController : Controller
    {
        ...
        public MeccanicheController(ILogger<MeccanicheController> logger, IConfiguration configuration, IOptions<PathExternal> external_lib)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            /*
            //Scrittura Ordinamento
            List<Meccaniche> mList = await this.ctx.Meccaniches.ToListAsync();
            string jsonStr = JsonConvert.SerializeObject(mList);
            using (StreamWriter sw = new StreamWriter("E:\\Dropbox\\Progetti\\Edro21\\Istanta\\Istanta\\Istanta\\wwwroot\\external_source\\SourceMeccaniche.json"))
            {
        ...
        public async Task<IActionResult> Index(Int16 id, Meccaniche item)
        {
            ...
            if (edit != null)
            {
                ...
                edit.Aree = item.Aree.Count > 0 ? item.Aree[0].Split(',').ToList() :new List<string>();
