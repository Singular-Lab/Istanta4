//Versione 1.1.0 Modificato il 19/01/2024 da Alessio (Aggiunta del filtro ricerca in Menabò)
//Versione 1.2.0 Modificato il 02/02/2024 da Alessio
...
class Agenzia extends IAgenzia    {
    ...
    Form_ricercaTracciato = '<option value="speciale">Speciale</option>' +
        ...
        $.getJSON('external_source/SourceOrdinamentoLista.json?v=1', function (data) {
            ...
            for (let i = 0; i < lista.length; i++) {
                ...
                for (let i = 0; i < data.source.length; i++) {
                    ...
                    for (var c = 0; c < item.chiavi.length; c++) {

                        var itemC = item.chiavi[c];
                        if (itemC.chiave != "categoria") {

                            if (itemC.chiave == "speciale")
                            {
                                if (itemC.valore == "" && itemListaRecord[itemC.chiave] != "") {
                                    break;
                                }
                                else if (itemC.valore == "!=" && itemListaRecord[itemC.chiave] == "") {
                                    break;
                                }
                            }
                            else {

                                var itemCinItemLista = itemListaRecord[itemC.chiave];
                                if (itemCinItemLista == null || itemCinItemLista != itemC.valore[0]) {
                                    //fallito
                                    break;
                                }
                            }
                            ...
                            //Siamo arrivati alla categoria, adesso vediamo se c'è un indice specifico, altrimenti riceve ultimo indice di questo settore reparto
                            if (itemC.valore.length > 1) {
                                for (var vc = 0; vc < itemC.valore.length; vc++) {
                                    var valCat = itemC.valore[vc];
                                    if (valCat.indexOf("CONTAINS$") >= 0) {
                                        valCat = valCat.replace("CONTAINS$", "");
                                        if (itemListaRecord[itemC.chiave].indexOf(valCat) >= 0) {
                                            inx = (50 * i ) + vc;
                                            break;
                                        }
                                    }
                                    else {
                                        if (itemListaRecord[itemC.chiave] == valCat) {
                                            inx = (50 * i) + vc;
                                            break;
                                        }
                                    }
                            ...
                            if (inx == -1) {
                                //Se non trovbo la categoria assegno l'ultimo indice del settore/reparto
                                inx = (50 * i) + itemC.valore.length;
                            }
                        }
                    }
            ...
            let _list_ordinata_per_speciale = lista.sort(function (a, b) {
    ...
    SetPrezzi(htmlItem, inTrac, areaTracciato, groupItem) {
        if (inTrac[keyRefCodice] != null) {
            htmlItem.find("#rigaPrezzoSingolo").css("display", "block");
            if (areaTracciato == 'MARKET') {
                htmlItem.find("#PrezzoSingle").val(inTrac.prezzo_promo);
            }
            else if (areaTracciato == 'ORO') {
                htmlItem.find("#PrezzoSingle").val(inTrac.prezzo_promo_oro);
            }
            else {
                htmlItem.find("#PrezzoSingle").val(inTrac.prezzo_promo_frattini);
            }

        }
        else {
            htmlItem.find("#rigaPrezziGruppo").css("display", "block");
            if (areaTracciato == 'MARKET') {
                const minPrezzoItem = groupItem.reduce((minItem, currentItem) => {
                    if (!minItem || currentItem.recordInTracciato.prezzo_promo < minItem.recordInTracciato.prezzo_promo) {
                        return currentItem;
                    }
                    return minItem;
                }, null);
                htmlItem.find("#PrezzoMinGruppo").val(minPrezzoItem.recordInTracciato.prezzo_promo);

                const maxPrezzoItem = groupItem.reduce((maxItem, currentItem) => {
                    if (!maxItem || currentItem.recordInTracciato.prezzo_promo > maxItem.recordInTracciato.prezzo_promo) {
                        return currentItem;
                    }
                    return maxItem;
                }, null);
                htmlItem.find("#PrezzoMaxGruppo").val(maxPrezzoItem.recordInTracciato.prezzo_promo);
            }
            else if (areaTracciato == 'ORO') {
                const minPrezzoItem = groupItem.reduce((minItem, currentItem) => {
                    if (!minItem || currentItem.recordInTracciato.prezzo_promo_oro < minItem.recordInTracciato.prezzo_promo_oro) {
                        return currentItem;
                    }
                    return minItem;
                }, null);
                htmlItem.find("#PrezzoMinGruppo").val(minPrezzoItem.recordInTracciato.prezzo_promo_oro);

                const maxPrezzoItem = groupItem.reduce((maxItem, currentItem) => {
                    if (!maxItem || currentItem.recordInTracciato.prezzo_promo_oro > maxItem.recordInTracciato.prezzo_promo_oro) {
                        return currentItem;
                    }
                    return maxItem;
                }, null);
                htmlItem.find("#PrezzoMaxGruppo").val(maxPrezzoItem.recordInTracciato.prezzo_promo_oro);
            }
            else {
                const minPrezzoItem = groupItem.reduce((minItem, currentItem) => {
                    if (!minItem || currentItem.recordInTracciato.prezzo_promo_frattini < minItem.recordInTracciato.prezzo_promo_frattini) {
                        return currentItem;
                    }
                    return minItem;
                }, null);
                htmlItem.find("#PrezzoMinGruppo").val(minPrezzoItem.recordInTracciato.prezzo_promo_frattini);

                const maxPrezzoItem = groupItem.reduce((maxItem, currentItem) => {
                    if (!maxItem || currentItem.recordInTracciato.prezzo_promo_frattini > maxItem.recordInTracciato.prezzo_promo_frattini) {
                        return currentItem;
                    }
                    return maxItem;
                }, null);
                htmlItem.find("#PrezzoMaxGruppo").val(maxPrezzoItem.recordInTracciato.prezzo_promo_frattini);
            }

        }
    }
    mostraFirmaTracciato() {
        //console.warn("Chiamata mostraFirmaTracciato non implementata");
        return false;
    }
}