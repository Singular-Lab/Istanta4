...
namespace Istanta.Controllers
{
    public class MenaboController : Controller
    {
        ...
        [HttpGet]
        [Route("Menabo/getFormatoNew/{meccanica}")]
        public async Task<IActionResult> getFormatoNew(string meccanica)
        {
            ExternalSourceClass exClass = new ExternalSourceClass(this.path_external_source, new string[] { "SourceDeclinazioneMeccaniche" });
            return Ok(exClass.getFormatoCombinazioneMeccanica(meccanica));
        }
        ...
        [HttpGet]
        [Route("Menabo/scaricaPagineSource/{id_tracciato}")]
        public async Task<IActionResult> scaricaPagineSource(int id_tracciato)
        {
            List<MenaboPagine> pags = await this.ctx2.MenaboPagines.Include(i => i.MenaboRefs).Where(m => m.IdTracciato == id_tracciato).ToListAsync();
            return Ok(pags);
        }
        ...
        //[HttpGet]
        //[Route("Menabo/getListaTracciato/{id}")]
        //public async Task<IActionResult> getListaTracciato(int id)
        //{
        ...
        //        PromoTracciati tracciato = await this.ctx2.PromoTracciatis.Include(i => i.PromoTracciatiRecords).Where(t => t.Id == id).FirstOrDefaultAsync();
        //        Dictionary<string, object> Area = JsonConvert.DeserializeObject<Dictionary<string, object>>(tracciato.Meta);
        //        result.AreaPromo = Area["Area"].ToString();
        //        List<MenaboRef> ref_impaginate = await this.ctx2.MenaboRefs.Where(mr => mr.IdPaginaNavigation.IdTracciato == id).ToListAsync();
        ...
        //        q_records.AddRange(tracciato.PromoTracciatiRecords.ToList());

        //        List<string> q_records_codici = q_records.Select(s => IstantaJson.getJsonObject(s.Dato)[keyCodiceRef].ToString()).ToList();
        ...
        //        codici_gruppi = q_records.GroupBy(g => IstantaJson.getValueOfJsonObject(g.Dato, key_codice_gruppo)).Select(s => s.Key.ToString()).ToList();
        //        //List<Dictionary<string, object>> tracciatoSingoli = new List<Dictionary<string, object>>();
        //        var tracciatoSingoli = q_records.GroupBy(g => IstantaJson.getJsonObject(g.Dato)[keyCodiceRef]).Select(s => new
        //        {
        //            Dato = IstantaJson.getJsonObject(s.FirstOrDefault().Dato),
        //            Id = s.FirstOrDefault().Id,
        //            StatoSelezione = s.FirstOrDefault().SelezioneMenabo,
        //            HasFoto = (returnHasFoto(IstantaJson.getJsonObject(s.FirstOrDefault().Dato)) ? AskFoto.FotoPresente : AskFoto.FotoNonPresente)
        //        }
        //        ).ToList();
        ...
        //                var objT = tracciatoSingoli.Where(t => t.Dato[keyCodiceRef].ToString() == codRef).FirstOrDefault();
        //                item.recordInTracciato = objT.Dato;
        ...
        //                item.isGruppo = false;
        //                item.idRec = objT.Id;
        //                item.statoSelezione = objT.StatoSelezione;
        //                item.hasFoto = (Byte)objT.HasFoto;
        //                item.recordImpaginato = ref_impaginate.Where(i => i.IdRecord == objT.Id).Select(s => new ArticoloImpaginato()
        //                {
        //                    CodiceGruppo = s.CodiceGruppo,
        //                    Formato = s.Formato,
        //                    Id = s.Id,
        //                    IdPagina = s.IdPagina,
        //                    IdRecord = s.IdRecord,
        ...
        //                var objT = tracciatoSingoli.Where(t => t.Dato.ContainsKey(key_codice_gruppo) && t.Dato[key_codice_gruppo]! == cod_gruppo && t.Dato.ContainsKey(scope_descrizioni!) && (t.Dato[scope_descrizioni!]! as JObject)!.ToObject<DescrittivaArticolo>()!.gruppo != null).FirstOrDefault();
        //                if (objT != null)
        //                {
        //                    itemG.recordInTracciato = objT.Dato;
        //                }
        //                else
        //                {
        ...
        //                itemG.recordImpaginato = ref_impaginate.Where(i => i.CodiceGruppo == cod_gruppo).Select(s => new ArticoloImpaginato()
        //                {
        //                    CodiceGruppo = s.CodiceGruppo,
        //                    Formato = s.Formato,
        //                    Id = s.Id,
...
double.TryParse(percorso.ToString(), out double valore16))
        ...
        [HttpPut]
        [Route("Menabo/inMenaboNew/{onoff}/{indice}/{idTracciato}/{areaTracciato}")]
        public async Task<IActionResult> inMenaboNew(MenaboRef obj, bool onoff, int idTracciato, string areaTracciato = "", Int16 indice = 0, bool automatismo = false, listaSetRegole? regole = null, DbEtichetta? etichetteDB = null, DbDeclinazioneMeccaniche? declMeccDB = null)
        {
            BoolResult result = new BoolResult();
            refsConMultiplex resultMultiplex = new refsConMultiplex();
            resultMultiplex.Ref = new MenaboRefLocal();
            ...
            try
            {
                ...
                if (onoff)
                {
                    if (etichetteDB!.source == null)
                    {
                        JObject? oEtichette = JObject.Parse(System.IO.File.ReadAllText(this.path_external_source + "SourceEtichetteRef.json"));
                        etichetteDB = oEtichette.ToObject<DbEtichetta>()!;
                    }

                    if (declMeccDB!.source == null)
                    {
                        JObject? oDeclMeccaniche = JObject.Parse(System.IO.File.ReadAllText(this.path_external_source + "SourceDeclinazioneMeccaniche.json"));
                        declMeccDB = oDeclMeccaniche.ToObject<DbDeclinazioneMeccaniche>()!;
                    }

                    string meccDefault = declMeccDB.meccanicaDefault;
                    if (_ref == null)
                    {
                        if (indice > 0)
                        {
                            if (!automatismo || regole == null)
                            {
                                ...
                                var recordPresenteAStessoIndice = await this.ctx2.MenaboRefs.Where(mp => mp.Id != _ref.Id && mp.IdPagina == obj.IdPagina && mp.Indice == indice).FirstOrDefaultAsync();
                                if (recordPresenteAStessoIndice == null)
                                {
                                    List<int> listIndexTaken = new(); //creo la lista di indici in cui salvare gli indici occupati
                                    foreach (MenaboRef mRef in pag.MenaboRefs)
                                    {
                                        IActionResult? formatoDaEstrarre = null;
                                        formatoDaEstrarre = await getFormatoNew(mRef.Formato!); //il formato in realtà è la meccanica
                                        string formato = "1x1";
                                        if (formatoDaEstrarre is OkObjectResult)
                                        {
                                            var okResult = formatoDaEstrarre as OkObjectResult;
                                    ...
                                    int iMeccanica = 0;
                                    RecordPostControlloMeccaniche recordConMeccaniche = new RecordPostControlloMeccaniche();
                                    List<int> lista1 = Enumerable.Range(1, indiceMax).ToList();
                                    lista1.RemoveAll(numero => listIndexTaken.Contains(numero));

                                    if (recordConMeccaniche.record == null)
                                    {
                                        recordConMeccaniche = IdentificaMeccanicaRecord(idTracciato, _ref);
                                    }
                                    else
                                    {
                                        recordConMeccaniche = IdentificaMeccanicaRecord(recordConMeccaniche.record);
                                    }

                                    do
                                    {
                                        string formatoRichiesto = recordConMeccaniche.meccanicheValide[iMeccanica].Formato;
                                        bool indiceValido = false;
                                        try
                                        {

                                            List<int> indiciRichiesti = CalcolaSpazi(pag.Formato, indice, formatoRichiesto);
                                            if (indiciRichiesti.Count > 0 && indiciRichiesti.All(f => lista1.Contains(f)))
                                            {
                                                indiceValido = true;
                                            }
                                        ...
                                        {
                                            //Inserisco la referenza
                                            _ref.Formato = recordConMeccaniche.meccanicheValide[iMeccanica].NomeCombinazione;

                                            if (_ref.Formato.Length > 100)
                                            {
                                                throw new Exception("La meccanica ha superato il limite di 100 caratteri");
                                            }
                                            _ref.Meccaniche = JsonConvert.SerializeObject(recordConMeccaniche.meccanicheValide);
                                            this.ctx2.MenaboRefs.Add(_ref);
                                            this.ctx2.SaveChanges();
                                            resultMultiplex.Ref.Selezione = _ref.Selezione;
                                            resultMultiplex.Ref.CodiceGruppo = _ref.CodiceGruppo;
                                            resultMultiplex.Ref.Meccaniche = recordConMeccaniche.meccanicheValide;
                                            resultMultiplex.Ref.Indice = _ref.Indice;
                                            resultMultiplex.Ref.Formato = _ref.Formato;
                                            resultMultiplex.Ref.Id = _ref.Id;
                                            resultMultiplex.Ref.IdPagina = _ref.IdPagina;
                                            resultMultiplex.Ref.IdRecord = _ref.IdRecord;
                                            resultMultiplex.Ref.IdPaginaNavigation = _ref.IdPaginaNavigation;
                                            resultMultiplex.allEtichette = recordConMeccaniche.record!.allEtichette;
                                            resultMultiplex.etichetteVisual = recordConMeccaniche.record.etichetteVisual;
                                            return Ok(resultMultiplex);
                                        }
                                        iMeccanica++;
                                    } while (iMeccanica < recordConMeccaniche.meccanicheValide.Count);
                                ...
                                else
                                {
                                    ...
                                    _ref.Formato = recordPresenteAStessoIndice.Formato;
                                    _ref.Meccaniche = JsonConvert.SerializeObject(recordConMeccaniche.meccanicheValide);
                                    this.ctx2.MenaboRefs.Add(_ref);
                                    this.ctx2.SaveChanges();
                                    resultMultiplex.Ref.Selezione = _ref.Selezione;
                        ...
                        else
                        {
                            ...
                            if (spaziOccupati < nSpazi)
                            {
                                ...
                                List<int> lista1 = Enumerable.Range(1, nSpazi).ToList();
                                lista1.RemoveAll(numero => listIndexTaken.Contains(numero));
                                foreach (var index in lista1)
                                {
                                    _ref.Indice = (Int16)index;
                                    if (recordConMeccaniche.record == null)
                                    {
                                        recordConMeccaniche = IdentificaMeccanicaRecord(idTracciato, _ref);
                                    }
                                ...
                                ProvaMeccanicaSuccessiva:
                                    string formatoRichiesto = recordConMeccaniche.meccanicheValide[iMeccanica].Formato;
                                    try
                                    {

                                        List<int> indiciRichiesti = CalcolaSpazi(pag.Formato, index, formatoRichiesto);
                                        if (indiciRichiesti.Count > 0 && indiciRichiesti.All(f => lista1.Contains(f)))
                                        {
                                            indice_disponibile = (Int16)index;
                                        }
                                    ...
                                    {
                                        if (iMeccanica < recordConMeccaniche.meccanicheValide.Count)
                                        {
                                            iMeccanica++;
                                            goto ProvaMeccanicaSuccessiva;
                                        }
                                        else if (index == lista1[lista1.Count - 1])
                                        {
                                            result.errorCode = ErrorCodes.SpazioInsufficienteInMenabò;
                                            throw new Exception("Indice disponibile non trovato");
                                        }
                            ...
                            {
                                #region impaginamento automatico
                                //if (automatismo && regole != null)
                                //{
                                //    if (regole.formatoPagina != "0")
                                //    {
                ...
                else
                {
                    if (_ref != null)
                    {
                        ...
                        RecordPostControlloMeccaniche recordConMeccaniche = IdentificaMeccanicaRecord(idTracciato, _ref, null, true);
                        resultMultiplex.Ref.Meccaniche = recordConMeccaniche.meccanicheValide;
                        resultMultiplex.allEtichette = recordConMeccaniche.record!.allEtichette;
                        resultMultiplex.etichetteVisual = recordConMeccaniche.record.etichetteVisual;
                        //Ok procedo alla rimozione
        ...
        public RecordPostControlloMeccaniche IdentificaMeccanicaRecord(int idTracciato, MenaboRef refRecord, DbDeclinazioneMeccaniche? meccDB = null, bool setRefToNull = false)
        {
            ...
            //                                    if ((meccanicheLiv == null && regola.presente)
            //                                        || (meccanicheLiv.Meccaniche.Find(f => f.Id == (long)regola.idMeccanica) == null && regola.presente)
            //                                        || (meccanicheLiv != null && meccanicheLiv.Meccaniche.Find(f => f.Id == (long)regola.idMeccanica) != null && !regola.presente))
            //                                    {
            //                                        setValido = false;
            //                                        break;
        ...
        //public RecordPostControlloMeccaniche IdentificaMastroRecord(int idTracciato, string codice, DbDeclinazioneMeccaniche meccDB = null, bool setRefToNull = false)
        //{
        //    string key_codice_gruppo = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppo;
        //    string keyCodiceRef = Enum.GetName(AddestramentoRuoli.Referenza) + "." + GLOBAL_VARIABLES.keyRefCodice;
        //    string primoCodice = "";
        ...
        //    if (!codice.Contains(","))
        //    {
        //        Articoli artItem = this.ctx.Articolis.Include(i => i.ArticoliDescrizionis).Where(r => r.Codice == tracciatoSingolo.Dato[keyCodiceRef]).FirstOrDefault();
        //        if (artItem != null)
        //        {
        //            item.recordRevisionato = artItem.ArticoliDescrizionis.OrderByDescending(o => o.DataUltimaRicezione).FirstOrDefault();
        //        }
        ...
        //        meccDB = oDeclMecc.ToObject<DbDeclinazioneMeccaniche>();
        //    }
        //    List<List<DeclinazioneMeccanica>> listaMeccanichePerLivello = new List<List<DeclinazioneMeccanica>>();
        //    if (meccDB != null)
        //    {
        //        var meccRaggruppate = meccDB.source.OrderBy(o => o.Livello).GroupBy(s => s.Livello);
        //        foreach (var gruppoLivello in meccRaggruppate)
        //        {
        //            List<DeclinazioneMeccanica> meccanicheLivello = new List<DeclinazioneMeccanica>();
        //            foreach (var etichetta in gruppoLivello)
        //            {
        //                if (etichetta.Regole != null)
        //                {
        //                    foreach (var setRegole in etichetta.Regole)
        ...
        //                            if (setValido)
        //                            {
        //                                meccanicheLivello.Add(etichetta);
        //                            }
        //                        }
        //                    }
        //                }
        //            }
        //            if (meccanicheLivello.Count > 0)
        //            {
        //                listaMeccanichePerLivello.Add(meccanicheLivello);
        //            }
        //            else
        ...
        //                if (meccDefaultLiv != null)
        //                {
        //                    meccanicheLivello.Add(meccDefaultLiv);
        //                    listaMeccanichePerLivello.Add(meccanicheLivello);
        //                }
        //            }
        //        }
        ...
        //    {
        //        //Composizione possibili Meccaniche
        //        var combinazioniNomine = GetCartesianProduct(listaMeccanichePerLivello);
...
double.TryParse(percorso.ToString(), out double valore16))
        ...
        [HttpPut]
        [Route("Menabo/salvaRefInMenabo/{idTracciato}")]
        public async Task<IActionResult> salvaRefInMenabo(MenaboRef obj, int idTracciato)
        {
            BoolResult result = new BoolResult();

            try
            {
                MenaboRef? _ref = await this.ctx2.MenaboRefs.Where(mp => mp.Id == obj.Id).FirstOrDefaultAsync();
                refsConMultiplex resultMultiplex = new refsConMultiplex();
                resultMultiplex.Ref = new MenaboRefLocal();
                if (_ref != null)
                {
                    Int64 idPagRef = _ref.IdPagina;
                    string? idOrCod = (_ref.IdRecord != null ? _ref.IdRecord.ToString() : _ref.CodiceGruppo);
                    MenaboPagine? pag = this.ctx2.MenaboPagines.Include(f => f.MenaboRefs).Where(f => f.Id == (Int64)_ref.IdPagina && f.IdTracciato == idTracciato).FirstOrDefault();
                    List<int> indexTaken = new List<int>();
                    string? formatoObj = "1x1";
                    List<int> indexOccupatiObj = new List<int>();
                    List<int> indexOccupatiObjMultiplex = new List<int>();
                    if (obj.Indice > 0)
                    {
                        foreach (var menaboRef in pag!.MenaboRefs)
                        {
                            if (menaboRef.Id == _ref.Id)
                            {
                                IActionResult? formatoDaEstrarre = null;
                                formatoDaEstrarre = await getFormato(menaboRef.Formato!); //il formato in realtà è la meccanica
                                if (formatoDaEstrarre is OkObjectResult)
                                {
                                    var okResult = formatoDaEstrarre as OkObjectResult;
                                    if (okResult!.Value is string)
                                    {
                                        formatoObj = okResult.Value as string;
                                    }

                                }
                                indexOccupatiObj = CalcolaSpazi(pag.Formato!, obj.Indice, formatoObj!);
                                if (indexOccupatiObj == null || indexOccupatiObj.Count == 0)
                                {
                                    result.errorCode = ErrorCodes.Generic;
                                    throw new Exception("Dati errati");
                                }
                            ...
                            else
                            {
                                ...
                                List<int> indexOccupati = CalcolaSpazi(pag.Formato!, menaboRef.Indice, formato!);
                                if (indexOccupati == null || indexOccupati.Count == 0)
                                {
                                    result.errorCode = ErrorCodes.Generic;
                                    throw new Exception("Dati errati");
                                }
                                indexTaken.AddRange(indexOccupati);
                                if (menaboRef.Indice == obj.Indice)
                                {
                                    indexOccupatiObjMultiplex.AddRange(indexOccupati);
                                }
                            }
                        }
                        if (indexOccupatiObjMultiplex.Count > 0)
                        {
                            indexOccupatiObj.RemoveAll(f => indexOccupatiObjMultiplex.Contains(f));
                        }
                        else
                        {
                            indexOccupatiObj.Remove((int)obj.Indice);
                        }
                        if (indexTaken.Intersect(indexOccupatiObj).ToList().Count > 0)
                        {
                            result.errorCode = ErrorCodes.Generic;
                            throw new Exception("L'ingombro dell'elemento impedisce lo spostamento");
                        }

                        if (indexTaken.Contains(obj.Indice) && pag.MenaboRefs.Where(mp => mp.Id != _ref.Id && mp.IdPagina == idPagRef && mp.Indice == obj.Indice).FirstOrDefault() == null)
                        {
                            result.errorCode = ErrorCodes.Generic;
                            throw new Exception("Indice richiesto non disponibile");
                        }
                    }
                    await RimuoviMultiplex(idPagRef, _ref.Indice, idOrCod!, idTracciato);
                    resultMultiplex.codiceMultiplexRimossi = "";
                    this.ctx2.SaveChanges();
                    if (await this.ctx2.MenaboRefs.Where(mp => mp.Id != _ref.Id && mp.IdPagina == idPagRef && mp.Indice == _ref.Indice).FirstOrDefaultAsync() != null)
                    {
                        string key_codice_multiplex = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppoMultiplex;
                        List<MenaboRef> refDaModificare = await this.ctx2.MenaboRefs.Where(p => p.Id != _ref.Id && p.IdPagina == idPagRef && p.Indice == _ref.Indice).ToListAsync();

                        MenaboRef? _frItem = refDaModificare.FirstOrDefault();
                        PromoTracciatiRecord? record = await this.ctx2.PromoTracciatiRecords.Where(p => (_frItem!.CodiceGruppo != null ? p.CodiceGruppo == _frItem.CodiceGruppo : p.Id == _frItem.IdRecord)).FirstOrDefaultAsync();
                    ...
                    if (obj.Indice > 0)
                    {
                        _ref.Indice = obj.Indice;
                    }
                    else
                    {
                        resultMultiplex.Ref.Meccaniche = (_ref.Meccaniche != null ? JsonConvert.DeserializeObject<JArray>(_ref.Meccaniche)!.ToObject<List<CombinazioniMeccaniche>>() : null);
                        this.ctx2.MenaboRefs.Remove(_ref);
                    }
                    this.ctx2.SaveChanges();
                    if (await this.ctx2.MenaboRefs.Where(mp => mp.Id != _ref.Id && mp.IdPagina == idPagRef && mp.Indice == obj.Indice).FirstOrDefaultAsync() != null)
                    {
                        string codiceMultiplexAggiunto = await AggiungiMultiplex(idPagRef, _ref.Indice, idOrCod!, idTracciato);
                        this.ctx2.SaveChanges();

                        string key_codice_multiplex = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppoMultiplex;
                        bool isGruppo = idOrCod!.Contains(",");
                        List<MenaboRef> refDaModificare = await this.ctx2.MenaboRefs.Where(p => p.IdPagina == idPagRef && p.Indice == _ref.Indice || (isGruppo ? p.CodiceGruppo == idOrCod : p.IdRecord == Int64.Parse(idOrCod))).ToListAsync();

                        //PromoTracciatiRecord record = await this.ctx2.PromoTracciatiRecords.Where(p => (refDaModificare[0].CodiceGruppo != null ? p.CodiceGruppo == refDaModificare[0].CodiceGruppo : p.Id == refDaModificare[0].IdRecord)).FirstOrDefaultAsync();


                        //Dictionary<string, object> Dato = JsonConvert.DeserializeObject<Dictionary<string, object>>(record.Dato);
                        if (codiceMultiplexAggiunto != null && codiceMultiplexAggiunto != "")
                        {
                            resultMultiplex.codiceMultiplexAggiunto = codiceMultiplexAggiunto;
                        }
        ...
        [HttpPut]
        [Route("Menabo/salvaRefInMenaboNew/{idTracciato}")]
        public async Task<IActionResult> salvaRefInMenaboNew(MenaboRef obj, int idTracciato)
        {
            BoolResult result = new BoolResult();
                ...
                if (_ref != null)
                {
                    ...
                    if (obj.Indice > 0)
                    {
                        foreach (var menaboRef in pag!.MenaboRefs)
                        {
                            ...
                            {
                                IActionResult? formatoDaEstrarre = null;
                                formatoDaEstrarre = await getFormatoNew(menaboRef.Formato!); //il formato in realtà è la meccanica
                                string? formato = "1x1";
                                if (formatoDaEstrarre is OkObjectResult)
                                {
                                    var okResult = formatoDaEstrarre as OkObjectResult;
                                ...
                                indexOccupatiObj = CalcolaSpazi(pag.Formato!, menaboRef.Indice, formato!);
                                if (indexOccupatiObj == null || indexOccupatiObj.Count == 0)
                                {
                                    result.errorCode = ErrorCodes.Generic;
                                    throw new Exception("Dati errati");
                            ...
                            else
                            {
                                ...
                                indexTaken.AddRange(indexOccupatiObj);
                                if (menaboRef.Indice == obj.Indice)
                                {
                                    indexOccupatiObjMultiplex.AddRange(indexOccupatiObj);
                                }
                            }
                        }
                        if (indexOccupatiObjMultiplex.Count > 0)
    ...
    .ToList();
        ...
        //[HttpGet]
        //[Route("Menabo/svuotaPagina/{idTracciato}/{numeroPagina}")]
        //public async Task<IActionResult> svuotaPagina(int idTracciato, int numeroPagina)
        //{
        //    BoolResult result = new BoolResult();
        //    bool modifiche = false;
        //    string key_codice_multiplex = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppoMultiplex;
        //    MenaboPagine pagSelected = await this.ctx2.MenaboPagines.Include(m => m.MenaboRefs).Where(p => p.IdTracciato == idTracciato && p.Numero == numeroPagina).FirstOrDefaultAsync();
        //    foreach (MenaboRef menaboRef in pagSelected.MenaboRefs)
        //    {
        //        try
        //        {
        //            //Ok procedo alla rimozione
        //            if (menaboRef.CodiceGruppo != null)
        //            {
        //                List<string> codici = new List<string>();
        //                codici.AddRange(menaboRef.CodiceGruppo.Split(","));
        //                foreach (string codice in codici)
        //                {
        //                    PromoTracciatiRecord record = await this.ctx2.PromoTracciatiRecords.Where(p => p.Codice == codice && p.IdTracciato == pagSelected.IdTracciato).FirstOrDefaultAsync();
        //                    Dictionary<string, object> Dato = JsonConvert.DeserializeObject<Dictionary<string, object>>(record.Dato);
        ...
        //            else
        //            {
        //                PromoTracciatiRecord record = await this.ctx2.PromoTracciatiRecords.Where(p => p.Id == menaboRef.IdRecord).FirstOrDefaultAsync();
        //                Dictionary<string, object> Dato = JsonConvert.DeserializeObject<Dictionary<string, object>>(record.Dato);
        //                Dato.Remove(key_codice_multiplex);
        //                record.Dato = JsonConvert.SerializeObject(Dato);
        //            }
        //            this.ctx2.MenaboRefs.Remove(menaboRef);
        ...
        [HttpGet]
        [Route("Menabo/inviaANuovaPagina/{idorCodice}/{id_cur_pag}/{id_pag}/{indice}/{formato}")]
        public async Task<IActionResult> inviaANuovaPagina(string idorCodice, int id_cur_pag, int id_pag, int indice, string formato)
        {
            BoolResult result = new BoolResult();
            try
            {
                ...
                if (isGruppo)
                {
                    refSelcted = await this.ctx2.MenaboRefs.Where(p => p.CodiceGruppo == idorCodice).FirstOrDefaultAsync();
                }
                else
                {
                    refSelcted = await this.ctx2.MenaboRefs.Where(p => p.IdRecord.ToString() == idorCodice).FirstOrDefaultAsync();
                }
                refSelcted!.Formato = formato;
                refSelcted.Indice = Convert.ToInt16(indice);
                refSelcted.IdPagina = id_pag;
                this.ctx2.SaveChanges();
                result.Esito = true;
            }
            catch (Exception ex)
        ...
        [HttpPut]
        [Route("Menabo/spostaAPaginaDiMassa/{id_pag}/{id_tracciato}")]
        public async Task<IActionResult> spostaAPaginaDiMassa(ArticoliImpaginati pkg, int id_pag, int id_tracciato)
        {
            BoolResult result = new BoolResult();
            try
            {
                MenaboPagine? pag = await this.ctx2.MenaboPagines.Where(m => m.IdTracciato == id_tracciato && m.Id == pkg.articoli![0].IdPagina).FirstOrDefaultAsync();
                await Index(id_tracciato, id_pag, pag!.IdMastro, pag.Formato!);
                foreach (ArticoloImpaginato art in pkg.articoli!)
                {
                    await inviaANuovaPagina((art.IdRecord != null ? art.IdRecord.ToString() : art.CodiceGruppo)!, (Int32)art.IdPagina, id_pag, art.Indice, art.Formato!);
                }
                result.Esito = true;
            }
            catch (Exception ex)
        ...
        [HttpPut]
        [Route("Menabo/salvaDettaglio")]
        public async Task<IActionResult> salvaDettaglio(dettaglioObj recInTracciato)
        {
            BoolResult result = new BoolResult();
            try
            {
                if (recInTracciato.type == "boolean")
                {
                    if (recInTracciato.value != null)
                    {
                        recInTracciato.valueBool = bool.Parse(recInTracciato.value);
                    }
                    else
                    {
                        recInTracciato.valueBool = false;
                    }
                }
                else if (recInTracciato.type == "string")
                {
                    if (recInTracciato.value != null)
                    {
                        recInTracciato.valueString = recInTracciato.value;
                    }
                    else
                    {
                        recInTracciato.valueString = "";
                    }
                }
                else if (recInTracciato.type == "number")
                {
                    if (recInTracciato.value != null)
                    {
                        recInTracciato.valueNumber = float.Parse(recInTracciato.value.Replace(",", "."));
                    }
                    else
                    {
                        recInTracciato.valueNumber = 0;
                    }
                }

                PromoTracciatiRecord? tracciato = await ctx2.PromoTracciatiRecords.Where(t => t.Id == (Int64)recInTracciato.id_rec).FirstOrDefaultAsync();
                Dictionary<string, object>? Dato = JsonConvert.DeserializeObject<Dictionary<string, object>>(tracciato!.Dato!);
                if (Dato![recInTracciato.chiave!] is Int64)
                {
                    Dato[recInTracciato.chiave!] = (Int64)(recInTracciato.valueNumber);
                    //tracciato.Dato = Dato.ToString();
                }
                if (Dato[recInTracciato.chiave!] is Int32)
                {
                    Dato[recInTracciato.chiave!] = (Int32)(recInTracciato.valueNumber);
                    //tracciato.Dato = Dato.ToString();
                }
                if (Dato[recInTracciato.chiave!] is Int16)
                {
                    Dato[recInTracciato.chiave!] = (Int16)(recInTracciato.valueNumber);
                    //tracciato.Dato = Dato.ToString();
                }
                if (Dato[recInTracciato.chiave!] is float)
                {
                    Dato[recInTracciato.chiave!] = recInTracciato.valueNumber;
                    //tracciato.Dato = Dato.ToString();
                }
                if (Dato[recInTracciato.chiave!] is Double)
                {
                    Dato[recInTracciato.chiave!] = (Double)(recInTracciato.valueNumber);
                    //tracciato.Dato = Dato.ToString();
                }
                if (Dato[recInTracciato.chiave!] is string)
                {
                    Dato[recInTracciato.chiave!] = recInTracciato.valueString!;
                    //tracciato.Dato = Dato.ToString();
                }
                if (Dato[recInTracciato.chiave!] is bool)
                {
                    Dato[recInTracciato.chiave!] = recInTracciato.valueBool;
                    //tracciato.Dato = Dato.ToString();
                }

                tracciato.Dato = JsonConvert.SerializeObject(Dato);
        ...
        [HttpPut]
        [Route("Menabo/modificaUscitaInPoP/{id_tracciato}")]
        public async Task<IActionResult> modificaUscitaInPoP(FuoriPoPList list, int id_tracciato)
        {
            BoolResult result = new BoolResult();
            try
            {
                foreach (FuoriPoPObj PoPEle in list.PoPElements!)
                {
                    PromoTracciatiRecord? record;

                    if (PoPEle.id!.Contains(","))
                    {
                        record = await this.ctx2.PromoTracciatiRecords.Where(p => p.CodiceGruppo == PoPEle.id && p.IdTracciato == id_tracciato).FirstOrDefaultAsync();

                    }
                    else
                    {
                        record = await this.ctx2.PromoTracciatiRecords.Where(p => p.Codice == PoPEle.id && p.IdTracciato == id_tracciato).FirstOrDefaultAsync();
                    }
                    if (record == null)
                    {
                        throw new Exception("Referenza mancante in PromoTracciatiRecord, incongruenza nel database");
                    ...
                    if (PoPEle.value == false)
                    {
                        Dato![GLOBAL_VARIABLES.keyFuoriPoP] = !PoPEle.value;
                    }
                    else
                    {
                        Dato!.Remove(GLOBAL_VARIABLES.keyFuoriPoP);
                    }
                    record.Dato = JsonConvert.SerializeObject(Dato);
                }
                this.ctx2.SaveChanges();
        ...
        [HttpGet]
        [Route("Menabo/trovaAltriTracciatiNellaStessaPromo/{id_tracciato}")]
        public async Task<IActionResult> trovaAltriTracciatiNellaStessaPromo(int id_tracciato)
        {
            BoolResult result = new BoolResult();
            PromoTracciati? currentPromo = await this.ctx2.PromoTracciatis.Where(f => f.Id == id_tracciato).FirstOrDefaultAsync();
            List<PromoTracciati> promoTracciati = await this.ctx2.PromoTracciatis.Where(f => f.IdPromo == currentPromo!.IdPromo && f.Id != currentPromo.Id).ToListAsync();
            return Ok(promoTracciati);
        }

        [HttpGet]
        [Route("Menabo/controllaPresenzaPagineTracciato/{id_tracciato}")]
        public async Task<IActionResult> controllaPresenzaPagineTracciato(int id_tracciato)
        {
            BoolResult result = new BoolResult();
            bool pagineInTracciato;
            try
            {
                pagineInTracciato = await this.ctx2.MenaboPagines.Where(f => f.IdTracciato == id_tracciato).FirstOrDefaultAsync() != null;
            }
            catch (Exception ex)
            {
                ex.ToString();
        ...
        [HttpGet]
        [Route("Menabo/trovaConflittiInDuplicazione/{id_tracciato_da_duplicare}/{id_tracciato}")]
        public async Task<IActionResult> trovaConflittiInDuplicazione(int id_tracciato_da_duplicare, int id_tracciato)
        {
            ReportDuplicazione result = new ReportDuplicazione();
            result.indici_paginaDaNonDuplicare = new();
            string key_codice_multiplex = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppoMultiplex;
            string report = "";
            List<string> codiciNonTrovati = new();
            listMenaboRefs refs = new();
            refs.RefList = new();
            List<MenaboPagine> pagineDiPartenza = await this.ctx2.MenaboPagines.Include(i => i.MenaboRefs).ThenInclude(f => f.IdRecordNavigation).Where(f => f.IdTracciato == id_tracciato_da_duplicare).ToListAsync();
            foreach (var item in pagineDiPartenza)
            {
                refs.RefList.AddRange(item.MenaboRefs);
            }
            if (pagineDiPartenza.Count <= 0)
            {
                result.error = "Nessuna pagina trovata nel menabò di partenza";
                result.esito = false;
                return Ok(result);
            }
            PromoTracciati? tracciato = await this.ctx2.PromoTracciatis.Include(i => i.PromoTracciatiRecords).Where(t => t.Id == id_tracciato).FirstOrDefaultAsync();
            List<PromoTracciatiRecord> q_records = new List<PromoTracciatiRecord>();
            q_records.AddRange(tracciato!.PromoTracciatiRecords.Where(f => f.Stato == (byte)StatoRecord.Attivo).ToList());

            List<PromoTracciatiRecord> filteredRecords = getElementPreviousVersion(q_records);
            // List<PromoTracciatiRecord> filteredRecords = q_records
            ...
            List<tmpObjInPagina> listIndiciAnalizzati = new List<tmpObjInPagina>();

            foreach (var item in refs.RefList)
            {
                if (listIndiciAnalizzati.Find(f => f.Indice == item.Indice && f.Pagina == item.IdPagina) != null)
                {
                    continue;
                }
                tmpObjInPagina tmpObj = new();
                tmpObj.Indice = item.Indice;
                tmpObj.Pagina = item.IdPagina;
                listIndiciAnalizzati.Add(tmpObj);
                List<MenaboRef> multiplexRefs;
                multiplexRefs = refs.RefList.Where(f => f.Indice == item.Indice && f.IdPagina == item.IdPagina).ToList();
                bool isMultiplex = multiplexRefs.Count > 1;
                if (isMultiplex)
                {
                    bool multiplexAnalized = false;
                    foreach (var singleRefMultiplex in multiplexRefs)
                    {

                        List<PromoTracciatiRecord> matchingRecord = q_records.Where(f => (singleRefMultiplex.CodiceGruppo != null ? f.CodiceGruppo == singleRefMultiplex.CodiceGruppo : f.Codice == singleRefMultiplex.IdRecordNavigation.Codice)).ToList();
                        List<PromoTracciatiRecord> outdatedMatchingRecord = filteredRecords.Where(f => (singleRefMultiplex.CodiceGruppo != null ? f.CodiceGruppo == singleRefMultiplex.CodiceGruppo : f.Codice == singleRefMultiplex.IdRecordNavigation.Codice)).ToList();

                        if (matchingRecord.Count == 0)
                        {
                            if (!multiplexAnalized)
                            {
                                multiplexAnalized = true;
                                string? codiceMultiplex;
                                // i gruppi non hanno idRecordNavigation
                                if (singleRefMultiplex.IdRecord != null)
                                {
                                    codiceMultiplex = IstantaJson.getJsonObject(singleRefMultiplex.IdRecordNavigation.Dato!)[key_codice_multiplex].ToString();
                                }
                                else
                                {
                                    string codice = singleRefMultiplex.CodiceGruppo!.Split(",")[0];
                                    //List<PromoTracciatiRecord> records = await this.ctx2.PromoTracciatiRecords.Where(t => t.IdTracciato == id_tracciato_da_duplicare).ToListAsync();
                                    PromoTracciatiRecord? record = await this.ctx2.PromoTracciatiRecords.Where(t => t.IdTracciato == id_tracciato_da_duplicare && t.Codice == codice).FirstOrDefaultAsync();
                                    codiceMultiplex = IstantaJson.getJsonObject(record!.Dato!)[key_codice_multiplex].ToString();
                                }

                                report += "Il multiplex<span class='text-danger'> " + codiceMultiplex + "</span> presente a pagina<span class='text-danger'> " + pagineDiPartenza!.Find(f => f.Id == singleRefMultiplex.IdPagina)!.Numero + "</span>, indice <span class='text-danger'>" + singleRefMultiplex.Indice + "</span> non sarà importato poichè:\n";
                                result.indici_paginaDaNonDuplicare.Add(singleRefMultiplex.Indice + "-" + singleRefMultiplex.IdPagina);
                            }
                            if (outdatedMatchingRecord.Count > 0)
                            {
                                string listaElementiRimastiAVersionePrecedente = "";
                                foreach (var codice in outdatedMatchingRecord)
                                {
                                    listaElementiRimastiAVersionePrecedente += codice.Codice + ", ";
                                }
                                listaElementiRimastiAVersionePrecedente = listaElementiRimastiAVersionePrecedente.Substring(0, listaElementiRimastiAVersionePrecedente.Length - 2);
                                report += (singleRefMultiplex.CodiceGruppo != null ? " - Il gruppo <span class='text-danger'>" : " - Il singolo<span class='text-danger'> ") + (singleRefMultiplex.CodiceGruppo != null ? singleRefMultiplex.CodiceGruppo : singleRefMultiplex.IdRecordNavigation.Codice) + "</span> non è presente nel nuovo tracciato" + (singleRefMultiplex.CodiceGruppo != null ? " poichè gli elementi <span class='text-danger'>" + listaElementiRimastiAVersionePrecedente + "</span> che lo componevano sono rimasti ad una versione precedente." : " poichè rimasto ad una versione precedente.") + "\n";
                            }
                            else
                            {
                                report += (singleRefMultiplex.CodiceGruppo != null ? " - Il gruppo <span class='text-danger'>" : " - Il singolo<span class='text-danger'> ") + (singleRefMultiplex.CodiceGruppo != null ? singleRefMultiplex.CodiceGruppo : singleRefMultiplex.IdRecordNavigation.Codice) + "</span> non è presente nel nuovo tracciato.\n";
                            }
                        }
                    }
                }
                ...
                {
                    List<PromoTracciatiRecord> matchingRecord = q_records.Where(f => (item.CodiceGruppo != null ? f.CodiceGruppo == item.CodiceGruppo : f.Codice == item.IdRecordNavigation.Codice)).ToList();
                    List<PromoTracciatiRecord> outdatedMatchingRecord = filteredRecords.Where(f => (item.CodiceGruppo != null ? f.CodiceGruppo == item.CodiceGruppo : f.Codice == item.IdRecordNavigation.Codice)).ToList();
                    if (matchingRecord.Count == 0 && outdatedMatchingRecord.Count > 0)
                    {
                        report += (item.CodiceGruppo != null ? "Il gruppo <span class='text-danger'>" : "Il singolo<span class='text-danger'> ") + (item.CodiceGruppo != null ? item.CodiceGruppo : item.IdRecordNavigation.Codice) + "</span> presente a pagina <span class='text-danger'>" + pagineDiPartenza.Find(f => f.Id == item.IdPagina)!.Numero + "</span>, indice <span class='text-danger'>" + item.Indice + "</span> non verrà importato, elemento non presente alla versione attuale.\n";
                        result.indici_paginaDaNonDuplicare.Add(item.Indice + "-" + item.IdPagina);
                    }
                    else if (matchingRecord.Count == 0 && outdatedMatchingRecord.Count == 0)
                    {
                        report += (item.CodiceGruppo != null ? "Il gruppo <span class='text-danger'>" : "Il singolo<span class='text-danger'> ") + (item.CodiceGruppo != null ? item.CodiceGruppo : item.IdRecordNavigation.Codice) + "</span> presente a pagina <span class='text-danger'>" + pagineDiPartenza.Find(f => f.Id == item.IdPagina)!.Numero + "</span>, indice <span class='text-danger'>" + item.Indice + "</span> non verrà importato, elemento mancante nel tracciato.\n";
                        result.indici_paginaDaNonDuplicare.Add(item.Indice + "-" + item.IdPagina);
                    }
                }
            }
        ...
        //[HttpPut]
        //[Route("Menabo/duplica/{id_tracciato_da_duplicare}/{id_tracciato}")]
        //public async Task<IActionResult> duplica(StringList indici_paginaDaNonDuplicare, int id_tracciato_da_duplicare, int id_tracciato)
        //{
        //    BoolResult result = new BoolResult();
        //    listMenaboRefs refs = new();
        //    refs.RefList = new();
        //    try
        //    {
        //        PromoTracciati tracciatoDiPartenza = await this.ctx2.PromoTracciatis.Include(i => i.PromoTracciatiRecords).Where(t => t.Id == id_tracciato_da_duplicare).FirstOrDefaultAsync();
        //        List<PromoTracciatiRecord> q_records_partenza = new List<PromoTracciatiRecord>();
        //        q_records_partenza.AddRange(tracciatoDiPartenza.PromoTracciatiRecords.ToList());

        //        q_records_partenza = getElementLastVersion(q_records_partenza);
        //        //q_records_partenza = q_records_partenza
        //        //.GroupBy(r => r.Label) // Raggruppa per label
        //        //.SelectMany(g => g.Where(r => r.Versione == g.Max(r => r.Versione))) // Seleziona gli elementi con la versione massima in ogni gruppo
        //        //.ToList();
        ...
        //            refs.RefList.AddRange(item.MenaboRefs);
        //        }
        //        foreach (string ind_pag in indici_paginaDaNonDuplicare.stringhe)
        //        {
        //            int indice = int.Parse(ind_pag.Split("-")[0]);
        //            int id_Pag = int.Parse(ind_pag.Split("-")[1]);
        //            refs.RefList.RemoveAll(f => f.Indice == indice && f.IdPagina == id_Pag);
        //        }
        //        List<MenaboPagine> pagineDiArrivo = await this.ctx2.MenaboPagines.Include(i => i.MenaboRefs).ThenInclude(f => f.IdRecordNavigation).Where(f => f.IdTracciato == id_tracciato).ToListAsync();
        //        foreach (var pagina in pagineDiArrivo)
        //        {
        //            await svuotaPagina(pagina.Id);
        //        }
        //        List<MenaboPagine> pagineDefinitive = new();
        //        foreach (var pagina in pagineDiPartenza)
        //        {
        //            if (pagineDiArrivo.Find(f => f.Numero == pagina.Numero) != null)
        //            {
        //                MenaboPagine pag = pagineDiArrivo.Find(f => f.Numero == pagina.Numero);
        //                await Index(pag.IdTracciato, pag.Id, pagina.IdMastro, pagina.Formato);
        //            }
        //            else
        //            {
        //                await Index(id_tracciato, 0, pagina.IdMastro, pagina.Formato);
        //            }
        //        }
        //        pagineDiArrivo = await this.ctx2.MenaboPagines.Include(i => i.MenaboRefs).ThenInclude(f => f.IdRecordNavigation).Where(f => f.IdTracciato == id_tracciato).ToListAsync();
        //        List<MenaboPagine> pagToDelate = new();
        //        foreach (var pag in pagineDiArrivo)
        //        {
        //            if (pagineDiPartenza.Find(f => f.Numero == pag.Numero) == null)
        //            {
        //                pagToDelate.Add(pag);
        //            }
        //        }
        //        pagineDiArrivo.RemoveAll(item => pagToDelate.Contains(item));
        //        ctx2.MenaboPagines.RemoveRange(pagToDelate);
        //        ctx2.SaveChanges();

        //        foreach (var menaboRef in refs.RefList)
        //        {
        //            MenaboRef refDaDuplicare = new MenaboRef();
        //            refDaDuplicare.CodiceGruppo = menaboRef.CodiceGruppo;
        //            refDaDuplicare.IdPagina = pagineDiArrivo.Find(f => f.Numero == menaboRef.IdPaginaNavigation.Numero).Id;
        //            refDaDuplicare.IdRecord = (refDaDuplicare.CodiceGruppo == null ? q_records.Find(f => f.Codice == menaboRef.IdRecordNavigation.Codice).Id : null);
        //            await inMenabo(refDaDuplicare, true, id_tracciato, "", menaboRef.Indice);
        //            if (refDaDuplicare.CodiceGruppo == null)
        //            {
        //                PromoTracciatiRecord rec = new();
        //                rec.Id = (long)refDaDuplicare.IdRecord;
        //                rec.SelezioneMenabo = q_records_partenza.Find(f => f.Id == menaboRef.IdRecord).SelezioneMenabo;
        //                await selezioneRefInMenabo(rec);
        //            }
        //            else
        //            {
        //                foreach (var item in refDaDuplicare.CodiceGruppo.Split(","))
        //                {
        //                    PromoTracciatiRecord rec = new();
        //                    rec.Id = q_records.Find(f => f.Codice == item).Id;
        //                    rec.SelezioneMenabo = q_records_partenza.Find(f => f.Codice == item).SelezioneMenabo;
        //                    await selezioneRefInMenabo(rec);
        //                }
        //            }
        //        }
        ...
        [HttpPut]
        [Route("Menabo/recuperaInformazioniArticoloInEliminazione")]
        public async Task<IActionResult> recuperaInformazioniArticoloInEliminazione(PromoTracciatiRecord rec)
        {
            BoolResult result = new BoolResult();
            try
            {
                //trova l'articolo per idRecord
                List<RecordConTracciato> recordResult = new();
                RecordConTracciato recordDaInserire = new();
                PromoTracciatiRecord? recordDiPartenza = await this.ctx2.PromoTracciatiRecords.Where(f => f.Id == rec.Id && f.Stato == (byte)StatoRecord.Attivo).FirstOrDefaultAsync();
                if (recordDiPartenza == null)
                {
                    throw new Exception("Il record che si sta cercando di eliminare non è presente nel tracciato attuale, discrepanza fra dato locale e server");
                }
                PromoTracciati? currentPromo = await this.ctx2.PromoTracciatis.Where(f => f.Id == rec.IdTracciato).FirstOrDefaultAsync();
                recordDaInserire.idTracciato = rec.IdTracciato;
                recordDaInserire.record = recordDiPartenza;
                recordDaInserire.siglaTracciato = currentPromo!.Sigla;
                recordResult.Add(recordDaInserire);

                //trova l'articolo negli altri tracciati in promo
                List<PromoTracciati> promoTracciati = await this.ctx2.PromoTracciatis.Where(f => f.IdPromo == currentPromo.IdPromo && f.Id != currentPromo.Id).ToListAsync();
                List<int> idTracciati = new();
                foreach (var prom in promoTracciati)
                {
                    idTracciati.Add(prom.Id);
                }
                List<PromoTracciatiRecord> recordGemelliDaInserire = await this.ctx2.PromoTracciatiRecords.Where(f => f.Id != rec.Id && idTracciati.Contains(f.IdTracciato) && f.Codice == rec.Codice && f.Stato == (byte)StatoRecord.Attivo).ToListAsync();
                foreach (var r in recordGemelliDaInserire)
                {
                    RecordConTracciato recordDaInserireGemello = new();
                    recordDaInserireGemello.record = r;
                    recordDaInserireGemello.idTracciato = r.IdTracciato;
                    recordDaInserireGemello.siglaTracciato = promoTracciati!.Where(f => f.Id == r.IdTracciato).FirstOrDefault()!.Sigla;
                    recordResult.Add(recordDaInserireGemello);
                }

                //recupera informazioni su impaginati
                for (int i = 0; i < recordResult.Count; i++)
                {
                    List<long> idsPagineTracciato = await this.ctx2.MenaboPagines.Where(f => f.IdTracciato == recordResult[i].idTracciato).Select(f => f.Id).ToListAsync();
                    var elementoImpaginato = await this.ctx2.MenaboRefs.Where(f => idsPagineTracciato.Contains(f.IdPagina) && (f.IdRecord == recordResult[i].record!.Id! || f.CodiceGruppo == recordResult[i].record!.CodiceGruppo)).FirstOrDefaultAsync();
                    if (elementoImpaginato != null)
                    {
                        if (elementoImpaginato.CodiceGruppo != null)
                        {
                            recordResult[i].impaginato = false;
                            recordResult[i].gruppoImpaginato = true;
                            recordResult[i].idPagina = (int)elementoImpaginato.IdPagina;
                        }
                        else
                        {
                            recordResult[i].impaginato = true;
                            recordResult[i].gruppoImpaginato = false;
                            recordResult[i].idPagina = (int)elementoImpaginato.IdPagina;
                        }
                    }
                    else
                ...
                for (int i = 0; i < recordResult.Count; i++)
                {
                    var dato = IstantaJson.getJsonObject(recordResult[i].record!.Dato!);
                    if (dato.ContainsKey(key_codice_multiplex))
                    {
                        recordResult[i].codiceMultiplex = dato[key_codice_multiplex].ToString()!;
                    }
                    else
                    {
                        recordResult[i].codiceMultiplex = "";
                    }
                }
                //tornare la lista di articoli con annesso il tracciato di appartenenza e la sigla
        ...
        [HttpPut]
        [Route("Menabo/eliminaArticoloManuale")]
        public async Task<IActionResult> eliminaArticoloManuale(RecordsPromo recs)
        {
            BoolResult result = new BoolResult();
            try
            {
                List<PromoTracciatiRecord> recordsDaRimuovere = new List<PromoTracciatiRecord>();
                //recupera records dal database di ogni recs
                foreach (var record in recs.records!)
                {
                    PromoTracciatiRecord? selectedRecord = await this.ctx2.PromoTracciatiRecords.Where(f => f.Id == record.Id).FirstOrDefaultAsync();
                    if (selectedRecord == null)
                    {
                        throw new Exception("Il record che si sta cercando non è più presente sul database, discrepanza fra il dato locale e il server");
                    }
                    recordsDaRimuovere.Add(selectedRecord);
                }
                //per ogni rec fare:
                //controllo se è impaginato, se lo è lo rimuovo dall'impaginato
                //string key_ref_codice = Enum.GetName(AddestramentoRuoli.Referenza) + "." + GLOBAL_VARIABLES.keyRefCodice;
                string key_codice_gruppo = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppo;

                foreach (PromoTracciatiRecord record in recordsDaRimuovere)
                {
                    List<long> idsPagineTracciato = await this.ctx2.MenaboPagines.Where(f => f.IdTracciato == record.IdTracciato).Select(f => f.Id).ToListAsync();
                    var elementoImpaginato = await this.ctx2.MenaboRefs.Where(f => idsPagineTracciato.Contains(f.IdPagina) && (f.IdRecord == record.Id || f.CodiceGruppo == record.CodiceGruppo)).FirstOrDefaultAsync();
                    if (elementoImpaginato != null)
                    {
                        _=inMenabo(elementoImpaginato!, false, record.IdTracciato, "", 0);
                    }

                    //controllo se fa parte di un gruppo, se lo è rimuovo il codice gruppo dalla rec (mettendolo uguale al codice), 
                    if (record.CodiceGruppo != record.Codice)
                    {
                        string codiceGruppo = record.CodiceGruppo!;
                        string nuovoCodiceGruppo = codiceGruppo!.ToString();
                        nuovoCodiceGruppo = nuovoCodiceGruppo.Replace(record.Codice!.ToString(), "");
                        nuovoCodiceGruppo = nuovoCodiceGruppo.Trim(',');
                        nuovoCodiceGruppo = nuovoCodiceGruppo.Replace(",,", ",");

                        //modifico il dato nel codice gruppo
                        var dato = IstantaJson.getJsonObject(record.Dato!);
                        dato[key_codice_gruppo] = record.Codice.ToString();
                        record.Dato = JsonConvert.SerializeObject(dato);
                        //recupero gli altri elementi del gruppo e imposto i loro codici gruppo di modo che non ci sia il codice della rec
                        List<PromoTracciatiRecord> recordsDelGruppo = await this.ctx2.PromoTracciatiRecords.Where(f => f.CodiceGruppo == codiceGruppo && f.Codice != record.Codice && f.IdTracciato == record.IdTracciato).ToListAsync();
                        foreach (var recordGr in recordsDelGruppo)
                        {
                            //modifico il dato nel codice gruppo
                            recordGr.CodiceGruppo = nuovoCodiceGruppo;
                            var datoRecordGr = IstantaJson.getJsonObject(recordGr.Dato!);
                            datoRecordGr[key_codice_gruppo] = nuovoCodiceGruppo;
                            recordGr.Dato = JsonConvert.SerializeObject(datoRecordGr);
                        }
                    }
                    //imposto lo stato della rec a 0 (disattivo)
                    record.Stato = (byte)StatoRecord.Disattivo;
                }
                //salvo tutti i cambiamenti assieme
                this.ctx2.SaveChanges();
                result.Esito = true;
            }
            catch (Exception ex)
        ...
        [HttpGet]
        [Route("Menabo/creazioneAddestramentoCsv/{id_tracciato}")]
        public async Task<IActionResult> creazioneAddestramentoCsv(int id_tracciato)
        {
            BoolResult result = new BoolResult();
            try
            {
                var listaDaAnalizzare = this.ctx2.Promos.Include(i1 => i1.PromoTracciatis).ThenInclude(i2 => i2.PromoTracciatiRecords).ThenInclude(i3 => i3.MenaboRefs!).ThenInclude(i4 => i4.IdPaginaNavigation).ToList();
                //var listaDaAnalizzare = this.ctx2.PromoTracciatis.Include(i2 => i2.PromoTracciatiRecords).ThenInclude(i3 => i3.MenaboRefs).ThenInclude(i4 => i4.IdPaginaNavigation).Where(f => f.Id == id_tracciato).ToList();
                var listaDiCorrispondenzePerGruppi = this.ctx2.MenaboRefs.Include(i => i.IdPaginaNavigation).ToList();
                List<csvDemo> recordsWithSeparatedColumns = new();
                string keyCodiceRef = Enum.GetName(AddestramentoRuoli.Referenza) + "." + GLOBAL_VARIABLES.keyRefCodice;
                string keyCodiceMult = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppoMultiplex;
                
                foreach (var promo in listaDaAnalizzare)
                {
                    foreach (var tracciato in promo.PromoTracciatis)
                    {
                        var jsonMetaObject = IstantaJson.getJsonObject(tracciato.Meta!);

                        foreach (var record in tracciato.PromoTracciatiRecords)
                        {
                            var jsonDatoObject = IstantaJson.getJsonObject(record.Dato!);

                            if (record.MenaboRefs!.Count > 0)
                            {
                                var separatedColumns = new csvDemo
                                {
                                    Date = promo.DataRegistrazione,
                                    Area = jsonMetaObject.ContainsKey("Area") ? jsonMetaObject["Area"].ToString() : null,
                                    Pagina = record.MenaboRefs.FirstOrDefault()!.IdPaginaNavigation.Numero, // Sostituisci con il nome effettivo della colonna
                                    Indice = record.MenaboRefs.FirstOrDefault()!.Indice, // Sostituisci con il nome effettivo della colonna
                                    CodiceGruppo = record.CodiceGruppo == jsonDatoObject[keyCodiceRef].ToString() ? "0" : record.CodiceGruppo,
                                    Codice = record.Codice,
                                    Label = record.Label,
                                    //CodiceMultiplex = jsonDatoObject.ContainsKey(keyCodiceMult) ? jsonDatoObject[keyCodiceMult].ToString().ToLower() : "0",
                                    // Aggiungi qui le colonne separate dal JSON
                                    Settore = jsonDatoObject.ContainsKey("settore") ? jsonDatoObject["settore"].ToString()!.ToLower() : null,
                                    Reparto = jsonDatoObject.ContainsKey("reparto") ? jsonDatoObject["reparto"].ToString()!.ToLower() : null,
                                    Categoria = jsonDatoObject.ContainsKey("categoria") ? jsonDatoObject["categoria"].ToString()!.ToLower() : null,
                                    Speciale = jsonDatoObject.ContainsKey("speciale") ? jsonDatoObject["speciale"].ToString()!.ToLower() : null,
                                    // Aggiungi altre colonne JSON separatamente secondo necessità
                                };
                                if (separatedColumns.Settore == null)
                                {
                                    separatedColumns.SettoreUnknown = true;
                                }
                                if (separatedColumns.Reparto == null)
                                {
                                    separatedColumns.RepartoUnknown = true;
                                }
                                if (separatedColumns.Speciale == null)
                                {
                                    separatedColumns.SpecialeUnknown = true;
                                }
                                if (separatedColumns.Categoria == null)
                                {
                                    separatedColumns.CategoriaUnknown = true;
                                }
                                recordsWithSeparatedColumns.Add(separatedColumns);
                            }
                            else
                            {
                                var gruppoImpaginato = listaDiCorrispondenzePerGruppi.Where(f => f.CodiceGruppo == record.CodiceGruppo && f.IdPaginaNavigation.IdTracciato == record.IdTracciato).FirstOrDefault();
                                if (gruppoImpaginato == null)
                                {
                                    continue;
                                }
                                ...
                                var separatedColumns = new csvDemo
                                {
                                    Date = promo.DataRegistrazione,
                                    Area = jsonMetaObject.ContainsKey("Area") ? jsonMetaObject["Area"].ToString() : null,
                                    Pagina = gruppoImpaginato.IdPaginaNavigation.Numero, // Sostituisci con il nome effettivo della colonna
                                    Indice = gruppoImpaginato.Indice, // Sostituisci con il nome effettivo della colonna
                                    Label = record.Label,
                                    // Aggiungi qui le colonne separate dal JSON
                                    Codice = record.Codice,
                                    CodiceGruppo = gruppoImpaginato.CodiceGruppo == jsonDatoObject[keyCodiceRef].ToString() ? "0" : gruppoImpaginato.CodiceGruppo,
                                    //CodiceMultiplex = jsonDatoObject.ContainsKey(keyCodiceMult) ? jsonDatoObject[keyCodiceMult].ToString().ToLower() : "0",
                                    Settore = jsonDatoObject.ContainsKey("settore") ? jsonDatoObject["settore"].ToString()!.ToLower() : null,
                                    Reparto = jsonDatoObject.ContainsKey("reparto") ? jsonDatoObject["reparto"].ToString()!.ToLower() : null,
                                    Categoria = jsonDatoObject.ContainsKey("categoria") ? jsonDatoObject["categoria"].ToString()!.ToLower() : null,
                            ...
                            //if (record.Stato == (byte)StatoRecord.Attivo)
                            //{
                            //    var separatedColumns = new csvDemo
                            //    {
                            //        Area = jsonMetaObject.ContainsKey("Area") ? jsonMetaObject["Area"].ToString() : null,
                            ...
                            //        Codice = record.Codice,
                            //        CodiceGruppo = record.CodiceGruppo == jsonDatoObject[keyCodiceRef].ToString() ? "0" : record.CodiceGruppo,
                            //        Settore = jsonDatoObject.ContainsKey("settore") ? jsonDatoObject["settore"].ToString() : null,
                            //        Reparto = jsonDatoObject.ContainsKey("reparto") ? jsonDatoObject["reparto"].ToString() : null,
                            //        Categoria = jsonDatoObject.ContainsKey("categoria") ? jsonDatoObject["categoria"].ToString() : null,
                            //        Speciale = jsonDatoObject.ContainsKey("speciale") ? jsonDatoObject["speciale"].ToString() : null,
                            //        // Aggiungi altre colonne JSON separatamente secondo necessità
                            //    };
                            //    recordsWithSeparatedColumns.Add(separatedColumns);
                            //}
                    ...
                    //}
                    //var tabella = this.ctx2.PromoTracciatiRecords.ToList();

                    //List<csvDemo> recordsWithSeparatedColumns = tabella.Select(record =>
                    //{
                    //    //var dato = IstantaJson.getJsonObject(record.Dato);
                    //    var jsonObject = IstantaJson.getJsonObject(record.Dato);
                    //    var separatedColumns = new csvDemo
                    //    {
                    //        Codice = record.Codice, // Sostituisci con il nome effettivo della colonna
                    //        CodiceGruppo = record.CodiceGruppo, // Sostituisci con il nome effettivo della colonna
                    //                                            // Aggiungi qui le colonne separate dal JSON
                    //        Settore = jsonObject.ContainsKey("settore") ? jsonObject["settore"].ToString() : null,
                    //        Prezzo = jsonObject.ContainsKey("prezzo") ? jsonObject["prezzo"].ToString() : null,
                    //        // Aggiungi altre colonne JSON separatamente secondo necessità
                    //    };
                    //    return separatedColumns;
                    //}).ToList();
                }
                recordsWithSeparatedColumns = modificaParametri(recordsWithSeparatedColumns);
                using (var writer = new StreamWriter("C:\\Users\\Sviluppatore3\\OneDrive\\Desktop\\csvProva\\fileCsvAddestramento.csv"))
                using (var csv = new CsvWriter(writer, new CsvConfiguration(System.Globalization.CultureInfo.InvariantCulture)))
                {
                    csv.WriteRecords(recordsWithSeparatedColumns);
                }
                result.Esito = true;
                return Ok(result);
            }
        ...
        [HttpPost]
        [Route("Menabo/creaFile")]
        public async Task<IActionResult> creaFile(string fileContent, string fileName)
        {
            // Crea un oggetto Blob con il tipo MIME appropriato
            try
            {
                byte[] reportBytes = Encoding.UTF8.GetBytes(fileContent);
                var blob = new MemoryStream(reportBytes);
        ...
        [HttpPut]
        [Route("Menabo/scorriPagina/{idTracciato}/{pagId}/{newInx}/{idOrCodGruppo}")]
        public async Task<IActionResult> scorriPagina(MenaboRef itemInMenabo, int idTracciato, int pagId, int newInx, string idOrCodGruppo)
        {
            BoolResult result = new BoolResult();
            try
            {
                ...
                List<int> indiciOccuppatiPostMovimento = new List<int>();
                List<int> indiciBasePerFormazioneMultiplex = new List<int>();
                ...
                List<int> indiciDisponibiliPostMovimento = new List<int>();
                for (int i = 1; i <= pagIndiceMax; i++)
                {
                    indiciDisponibiliPostMovimento.Add(i);
                }
                List<MenaboRef> pagMenaboCopy = new List<MenaboRef>();
                //pagMenaboCopy.AddRange(pag.MenaboRefs.ToList());
                pagMenaboCopy.AddRange(pag.MenaboRefs.Select(item => new MenaboRef()).ToList());

                for (int i2 = 0; i2 < pagMenaboCopy.Count && i2 < pag.MenaboRefs.Count; i2++)
                {
                    pagMenaboCopy[i2].Indice = pag.MenaboRefs.ToList()[i2].Indice;
                    pagMenaboCopy[i2].Formato = pag.MenaboRefs.ToList()[i2].Formato;
                    pagMenaboCopy[i2].Id = pag.MenaboRefs.ToList()[i2].Id;
                }
                pagMenaboCopy = pagMenaboCopy.OrderBy(f => f.Indice).ToList();
                int findedMultiplexRefNumber = 0;
                bool primoElementoMultiplex = false;
                int indiceMultiplex = 0;
                foreach (var item in pagMenaboCopy)
                {
                    if (findedMultiplexRefNumber > 0)
                    {
                        findedMultiplexRefNumber--;
                    }

                    if (findedMultiplexRefNumber == 0)
                    {
                        findedMultiplexRefNumber = pagMenaboCopy.Where(f => f.Indice == item.Indice).Count();
                        if (findedMultiplexRefNumber > 1)
                        {
                            primoElementoMultiplex = true;
                            indiceMultiplex = 0;
                        }
                        else
                        {
                            primoElementoMultiplex = false;
                            indiceMultiplex = 0;
                        }
                    }
                    else
                    ...
                    if (!primoElementoMultiplex && indiceMultiplex != 0)
                    {
                        pag.MenaboRefs.Where(f => f.Id == item.Id).FirstOrDefault()!.Indice = (short)indiceMultiplex;
                        continue;
                    }
                    ...
                    if (item.Indice == itemInMenabo.Indice) //da mettere il controllo sull'indice originale
                    {
                        continue;
                    }
                    else if (item.Indice < newInx)
                    {
                        List<int> spaziRichiesti = CalcolaSpazi(pag.Formato, item.Indice, formatoRichiesto);
                        if (spaziRichiesti.Count == 0)
                        {
                            throw new Exception("Movimento impossibile, discrepanza nel dato");
                        }
                        List<int> elementiComuni = indiciOccuppatiPostMovimento.Intersect(spaziRichiesti).ToList();
                        if (elementiComuni.Count == 0)
                        {
                            pag.MenaboRefs.Where(f => f.Id == item.Id).FirstOrDefault()!.Indice = item.Indice;
                            indiciOccuppatiPostMovimento.AddRange(spaziRichiesti);
                            indiciDisponibiliPostMovimento.RemoveAll(item => spaziRichiesti.Contains(item));
                        }
                        else
                        {
                            throw new Exception("Movimento impossibile, discrepanza nel dato");
                    ...
                    {
                        List<int> spaziRichiesti = new List<int>();
                        List<int> elementiComuni = new List<int>();
                        if (itemInMenabo.Id != 0)
                        {
                            IActionResult? formatoResItemPartenza = null;
                            formatoResItemPartenza = await getFormato(itemInMenabo.Formato!);
                            string formatoRichiestoItemPartenza = "";
                            if (formatoResItemPartenza is OkObjectResult && (formatoResItemPartenza as OkObjectResult)!.Value is string)
                            {
                                formatoRichiestoItemPartenza = (formatoResItemPartenza as OkObjectResult)!.Value!.ToString()!;
                            }

                            spaziRichiesti = CalcolaSpazi(pag.Formato, newInx, formatoRichiestoItemPartenza!);
                            if (spaziRichiesti.Count == 0)
                            {
                                throw new Exception("Movimento impossibile, se posizionato ad indice " + newInx + " l'elemento uscirebbe dalla griglia a causa dell'ingombro.");
                            }
                            elementiComuni = indiciOccuppatiPostMovimento.Intersect(spaziRichiesti).ToList();
                            if (elementiComuni.Count == 0)
                            {
                                var listElementsSpostati = pag.MenaboRefs.Where(f => f.Indice == itemInMenabo.Indice).ToList();
                                foreach (var itemSpostato in listElementsSpostati)
                                {
                                    itemSpostato.Indice = item.Indice;
                                }
                                indiciOccuppatiPostMovimento.AddRange(spaziRichiesti);
                                indiciDisponibiliPostMovimento.RemoveAll(item => spaziRichiesti.Contains(item));
                            ...
                            {
                                throw new Exception("Movimento impossibile, non è possibile spostare l'elemento all'interno dell'ingombro di un altro");
                            }
                        }

                        int prossimoIndex = indiciDisponibiliPostMovimento.Where(x => x > newInx).DefaultIfEmpty().Min();
                        while (prossimoIndex != 0 && prossimoIndex <= pagIndiceMax)
                        {
                            spaziRichiesti = CalcolaSpazi(pag.Formato, prossimoIndex, formatoRichiesto);
                            if (spaziRichiesti.Count == 0)
                            {
                                indiciDisponibiliPostMovimento.Remove(prossimoIndex);
                                indiciOccuppatiPostMovimento.Add(prossimoIndex);
                                prossimoIndex = indiciDisponibiliPostMovimento.Where(x => x > newInx).DefaultIfEmpty().Min();
                                continue;
                            }
                            elementiComuni = indiciOccuppatiPostMovimento.Intersect(spaziRichiesti).ToList();
                            if (elementiComuni.Count == 0)
                            {
                                pag.MenaboRefs.Where(f => f.Id == item.Id).FirstOrDefault()!.Indice = (short)prossimoIndex;
                                indiciOccuppatiPostMovimento.AddRange(spaziRichiesti);
                                indiciDisponibiliPostMovimento.RemoveAll(item => spaziRichiesti.Contains(item));
                                if (primoElementoMultiplex)
                                {
                                    indiceMultiplex = prossimoIndex;
                                }
                                break;
                            }
                            else
                        ...
                        if (prossimoIndex == 0)
                        {
                            throw new Exception("Movimento impossibile, non è possibile far scorrere gli elementi impaginati per fare spazio");
                        }
                    }
                    else if (item.Indice > newInx)
                    {
                        int prossimoIndex = item.Indice;
                        while (prossimoIndex != 0 && prossimoIndex <= pagIndiceMax)
                        {
                            List<int> spaziRichiesti = CalcolaSpazi(pag.Formato, prossimoIndex, formatoRichiesto);
                            if (spaziRichiesti.Count == 0)
                            {
                                indiciDisponibiliPostMovimento.Remove(prossimoIndex);
                                indiciOccuppatiPostMovimento.Add(prossimoIndex);
                        ...
                        if (filtro.listFiltri != null)
                        {
                            ...
                            foreach (var fil in filtro.listFiltri)
                            {
                                ...
                                foreach (var regola in fil.Criteri!)
                                {
                                    ...
                                    //List<string> valori = regola.Valore.Split("||").ToList();
                                    ...
                                    //            criterioValido.AddRange(listaFiltrata.Where(f => f.ContainsKey(regola.Chiave) && valori.Any(val => f[regola.Chiave].ToString().ToLower() == val.ToString().ToLower()));
                                    //        }
                                    //        else
                                    //        {
                                    //            criterioValido.RemoveAll(f => !f.ContainsKey(regola.Chiave) || f[regola.Chiave].ToString().ToLower() != regola.Valore.ToString().ToLower());
                                    //        }
                                    //        break;
                                    ...
                                    //        {
                                    //            firstIterationCriterio = false;
                                    //            criterioValido.AddRange(listaFiltrata.Where(f =>
                                    //            {
                                    //                if (f.ContainsKey(regola.Chiave) && float.TryParse(f[regola.Chiave].ToString(), out float datoValue) &&
                                    //                    float.TryParse(regola.Valore.ToString(), out float regolaValue))
                                    //                {
                                    //                    return datoValue < regolaValue;
                                    //                }
                                    //                return false;
                                    //            }));
                                    //        }
                                    ...
                                    //        {
                                    //            firstIterationCriterio = false;
                                    //            criterioValido.AddRange(listaFiltrata.Where(f => f.ContainsKey(regola.Chiave) && f[regola.Chiave].ToString().ToLower() != regola.Valore.ToString().ToLower()));
                                    //        }
                                    //        else
                                    //        {
                                    //            criterioValido.RemoveAll(f => !f.ContainsKey(regola.Chiave) || f[regola.Chiave].ToString().ToLower() == regola.Valore.ToString().ToLower());
                                    ...
                                    //        {
                                    //            firstIterationCriterio = false;
                                    //            criterioValido.AddRange(listaFiltrata.Where(f => f.ContainsKey(regola.Chiave) && f[regola.Chiave].ToString().ToLower().Contains(regola.Valore.ToString().ToLower())));
                                    //        }
                                    //        else
                                    //        {
                                    //            criterioValido.RemoveAll(f => !f.ContainsKey(regola.Chiave) || !f[regola.Chiave].ToString().ToLower().Contains(regola.Valore.ToString().ToLower()));
                                    //        }
                                    //        break;
    ...
    .ToList();
                                    ...
                                    //            firstIterationCriterio = false;
                                    //            criterioValido.AddRange(listaFiltrata.Where(f =>
                                    //                f.ContainsKey(regola.Chiave!) &&
                                    //                valori.Any(val => EqualsValue(f[regola.Chiave!], val))
                                    //            ));
                                    //        }
                                    //        else
                                    //        {
                                    ...
                                    //            criterioValido.AddRange(listaFiltrata.Where(f =>
                                    //                f.ContainsKey(regola.Chiave!) &&
                                    //                valori.Any(val => CompareValue(f[regola.Chiave!], val, Operatore.Minore))
                                    //            ));
                                    //        }
                                    //        else
                                    //        {
                                    ...
                                    //            criterioValido.AddRange(listaFiltrata.Where(f =>
                                    //                f.ContainsKey(regola.Chiave!) &&
                                    //                valori.Any(val => CompareValue(f[regola.Chiave!], val, Operatore.MinoreUguale))
                                    //            ));
                                    //        }
                                    //        else
                                    //        {
                                    ...
                                    //            criterioValido.AddRange(listaFiltrata.Where(f =>
                                    //                f.ContainsKey(regola.Chiave!) &&
                                    //                valori.Any(val => CompareValue(f[regola.Chiave!], val, Operatore.Maggiore))
                                    //            ));
                                    //        }
                                    //        else
                                    //        {
                                    ...
                                    //            criterioValido.AddRange(listaFiltrata.Where(f =>
                                    //                f.ContainsKey(regola.Chiave!) &&
                                    //                valori.Any(val => CompareValue(f[regola.Chiave!], val, Operatore.MaggioreUguale))
                                    //            ));
                                    //        }
                                    //        else
                                    //        {
                                    ...
                                    //        {
                                    //            firstIterationCriterio = false;
                                    //            criterioValido.AddRange(listaFiltrata.Where(f => f.ContainsKey(regola.Chiave!) && valori.Any(val => f[regola.Chiave!].ToString()!.ToLower().Contains(val))));
                                    //        }
                                    //        else
                                    //        {
                                    //            criterioValido.RemoveAll(f => !f.ContainsKey(regola.Chiave!) || !valori.Any(val => f[regola.Chiave!].ToString()!.ToLower().Contains(val)));
                                    //        }
                                    //        break;
    ...
    .FirstOrDefault(c => MatchCodiceFiltro(oggetto, c, key_codice_gruppo));
                                ...
                                while (!posizioneValida)
                                {
                                    ...
                                    if (!riempito)
                                    {
                                        ...
                                        if (boxDisponibile != -1)
                                        {
                                            ...
                                            //filtroCorrispondente.Griglia.Righe[resRicerca.posizioneRichiesta[1]][resRicerca.posizioneRichiesta[0]] = 1;
                                            filtroCorrispondente.Griglia.boxOccupati.Add(boxDisponibile);
                                            //var informazioneGenericaTmp = new int[2];
                                            //try
                                            //{
                                            //    bool elementiEspansi = false;
                                            //    foreach (var riga in filtroCorrispondente.Griglia.Righe)
                                            //    {
                                            //        foreach (var valoreCella in riga)
                                            //        {
                                            //            if (valoreCella == 2)
                                            //            {
                                            //                elementiEspansi = true;
                                            //                break;
                                            //            }
                                            //        }
                                            //        if (elementiEspansi)
                                            ...
                                            //        }
                                            //    }
                                            //    if (tipoImpaginazione == TipoImpagnazione.ImpaginaTutto && !elementiEspansi)
                                            //    {
                                            //        for (int repetition = 0; repetition < resRicerca.spaziOccupati - 1; repetition++)
                                            //        {

                                            //            var lastSpace = findLastSpaceGriglia(filtroCorrispondente.Griglia);
                                            //            informazioneGenericaTmp = lastSpace;
                                            //            filtroCorrispondente.Griglia.Righe[lastSpace[1]][lastSpace[0]] = 1;
                                            //        }
                                            //    }
                                            ...
                                            //catch (Exception ex)
                                            //{
                                            //    var deb = new DebugInterface();
                                            //    deb.oggettoConErrore = oggetto;
                                            //    deb.error = ex.ToString();
                                            //    deb.informazioneIntGenerica1 = informazioneGenericaTmp[0];
                                            //    deb.informazioneIntGenerica2 = informazioneGenericaTmp[1];
                                            //    deb.filtroCorrispondente = filtroCorrispondente;
                                            //    return Ok(deb);
                                            //}
                                        }
                                        else
        ...
        //private List<FotoElementoGruppo> getFotoSecondarieGruppo(List<Dictionary<string, object>> gruppo, List<PromoLavorazioniRecord> plrs)
        //{
        //    string keyXMLSelezione = GLOBAL_VARIABLES.keyXMLSelezione;
        //    string keyMembriGruppoFoto = GLOBAL_VARIABLES.keyMembriGruppoFoto;
        //    string keyNomeFoto = Enum.GetName(AddestramentoRuoli.Foto) + "." + GLOBAL_VARIABLES.keyFotoNome;
        ...
        //    try
        //    {
        //        RevisioneMetaPromoLavorazioni? metaInLavorazione = null;

        //        if (plrs.Count > 0)
        //        {
        //            metaInLavorazione = JsonConvert.DeserializeObject<RevisioneMetaPromoLavorazioni>(plrs.FirstOrDefault().Meta);
        //        }

        //        List<Dictionary<string, object>> nonPrimari = gruppo.Where(f => Byte.Parse(f[keyXMLSelezione].ToString()!) != (Byte)TipoSelezioneMenabo.Primaria).ToList();
        //        foreach (var singolo in nonPrimari)
        //        {
        //            FotoElementoGruppo el = new FotoElementoGruppo();
        //            el.nomeFoto = singolo[keyNomeFoto].ToString()!;
        //            el.codRef = singolo[key_codiceRef].ToString()!;
        //            el.statoSelezione = Byte.Parse(singolo[keyXMLSelezione].ToString()!);

        //            if (el.nomeFoto.IndexOf("$p=") > 0)
        //            {
        //                //Significa che l'istruzione nome foto di questo articolo presenta un puntamento al Primario.
        //                //Per questa esigenza però, ovvero registrarsi come foto secondaria, deve rispettare la sua VERA foto
        //                el.nomeFoto = el.nomeFoto.Substring(0, el.nomeFoto.IndexOf("$p="));
        //            }

        //            if (metaInLavorazione != null)
        //            {
        //                RevisioneSelezioneFotoFromIndd revPS = metaInLavorazione.ps.FirstOrDefault(p => p.codRef == el.codRef);
        //                if (revPS != null)
        //                {
        //                    el.statoSelezione = (Byte)revPS.stato;
        //                }
        //            }

        //            membriGruppoFoto.Add(el);
        //        }

        //    }
        //    catch (Exception ex)
        //    {
        //        Debug.WriteLine("Errore in getFotoSecondarieGruppo: " + ex.ToString());
        //    }

        //    return membriGruppoFoto;
        //}
        ...
        //public grigliaCheck CheckGrigliaPosition(Griglia griglia, List<int[]> posizioniEscluse, int[] posizioneCercata, int formatoColonne, int formatoRighe)
        //{
        //    grigliaCheck result = new grigliaCheck();
        //    result.posizioniRichieste = new List<int[]>();
        //    int lunghezzaRiga = griglia.Righe[posizioneCercata[1]].Count;
        //    for (int scorriIndiceRiga = 0; scorriIndiceRiga < formatoColonne; scorriIndiceRiga++)
        //    {
        //        for (int scorriIndiceColonne = 0; scorriIndiceColonne < formatoRighe; scorriIndiceColonne++)
        //        {   
        //            if ((griglia.Righe.Count <= posizioneCercata[1] + scorriIndiceColonne) /*abbiamo richiesto una riga non presente*/ || (griglia.Righe[posizioneCercata[1] + scorriIndiceColonne].Count <= posizioneCercata[0] + scorriIndiceRiga) /*abbiamo richiesto una colonna non presente*/ || (griglia.Righe[posizioneCercata[1] + scorriIndiceColonne][posizioneCercata[0] + scorriIndiceRiga] != 0))
        //            {
        //                result.esito = false;
        //                result.posizioniRichieste.Clear();
        //                return result;
        //            }
        //            else
        //            {
        //                if (lunghezzaRiga != griglia.Righe[posizioneCercata[1] + scorriIndiceColonne].Count) //si sta cercando di estenderci tra due righe con lunghezze differenti
        //                {
        //                    result.esito = false;
        //                    result.posizioniRichieste.Clear();
        //                    return result;
        ...
        //                if (posizioniEscluse.Any(posizione => posizione[0] == (posizioneCercata[0] + scorriIndiceRiga) && posizione[1] == posizioneCercata[1] + scorriIndiceColonne))
        //                {
        //                    result.esito = false;
        //                    result.posizioniRichieste.Clear();
        //                    return result;
        //                }
        //                result.posizioniRichieste.Add(new int[] { posizioneCercata[0] + scorriIndiceRiga, posizioneCercata[1] + scorriIndiceColonne });
        //            }
        //        }
        //    }
        //    result.esito = true;
        ...
        //public grigliaCheck CheckGrigliaPosition(Griglia griglia, List<int[]> posizioniEscluse, int[] posizioneCercata, int formatoColonne, int formatoRighe, TipoImpagnazione tipoImp)
        //{
        //    List<int> valValidi = new List<int>();

        //    if (tipoImp == TipoImpagnazione.ImpaginaTutto)
        //    {
        //        valValidi.Add(0);
        //        valValidi.Add(2);
        //    }
        //    else if (tipoImp == TipoImpagnazione.SoloIngombranti)
        //    {
        //        valValidi.Add(2);
        //    }
        //    else
        ...
        //    grigliaCheck result = new grigliaCheck();
        //    result.posizioneRichiesta = new int[] { -1, -1 };
        //    int lunghezzaRiga = griglia.Righe[posizioneCercata[1]].Count;
        //    int spaziRichiesti = 1;
        //    if (tipoImp == TipoImpagnazione.ImpaginaTutto)
        //    {
        //        spaziRichiesti = formatoColonne * formatoRighe;
        //    }
        //    int spaziLiberi = 0;
        //    if (valValidi.Contains(griglia.Righe[posizioneCercata[1]][posizioneCercata[0]]))
        //    {
        //        if (posizioniEscluse.Any(posizione => posizione[0] == posizioneCercata[0] && posizione[1] == posizioneCercata[1]))
        //        {
        //            result.esito = false;
        //            return result;
        //        }

        //        for (int scorriIndiceRiga = 0; scorriIndiceRiga < griglia.Righe.Count; scorriIndiceRiga++)
        //        {
        //            for (int scorriIndiceColonne = 0; scorriIndiceColonne < griglia.Righe[scorriIndiceRiga].Count; scorriIndiceColonne++)
        //            {
        //                if (valValidi.Contains(griglia.Righe[scorriIndiceRiga][scorriIndiceColonne]))
        //                {
        //                    spaziLiberi++;
        //                }
        //            }
        ...
        //        result.esito = true;
        //        result.posizioneRichiesta = posizioneCercata;
        //        result.spaziOccupati = spaziRichiesti;
        //        return result;
        //    }
        ...
        //public int[] findLastSpaceGriglia(Griglia griglia)
        //{
        //    for (int i = griglia.Righe.Count - 1; i >= 0; i--)
        //    {
        //        for (int col = griglia.Righe[i].Count - 1; col >= 0; col--)
        //        {
        //            if (griglia.Righe[i][col] == 0)
        //            {
        ...
        //[HttpGet]
        //[Route("Menabo/LeggiAllPromoRecords/{idPromo}/{importa}/{isPop}/{getMastro}/{getOnlyMeta}/{leggiDaOgniTracciato}")]
        //public async Task<IActionResult> LeggiAllPromoRecords(int idPromo, bool importa = true, bool isPop = false, bool getMastro = false, bool getOnlyMeta = false, bool leggiDaOgniTracciato = false)
        //{

        //    var promo = this.ctx2.Promos.Include(f => f.PromoTracciatis).Where(f => f.Id == idPromo).FirstOrDefault();
        //    Dictionary<string, object> promo_e_tracciati = new Dictionary<string, object>();
        //    if (promo == null)
        //    {
        //        BoolResult res = new BoolResult();
        //        res.error = "Promo non trovata";
        //        res.Esito = false;
        //        return Ok(res);
        //    }

        //    promo_e_tracciati.Add("promo", promo);

        //    this.ctx2 = new Edro21_DbContext2(_config.GetConnectionString("IstandaConnectionDb")!);

        //    foreach (var tracciato in promo.PromoTracciatis)
        //    {
        //        var tracciatoScaricato = await LeggiAllTracciatoRecord(tracciato.Id, importa, isPop, getMastro, getOnlyMeta, leggiDaOgniTracciato);
        //        if (tracciatoScaricato is OkObjectResult)
        //        {
        //            var okResult = tracciatoScaricato as OkObjectResult;
        //            if (okResult!.Value is Dictionary<string, object>)
        //            {
        //                promo_e_tracciati.Add(tracciato!.Sigla!, okResult.Value);
        //            }

        //        }
        //    }
        //    return Ok(promo_e_tracciati);
        //}


        //[HttpGet]
        //[Route("Menabo/LeggiAllTracciatoRecord/{idTracciato}/{importa}/{isPop}/{getMastro}/{getOnlyMeta}/{leggiDaOgniTracciato}")]
        //public async Task<IActionResult> LeggiAllTracciatoRecord(int idTracciato, bool importa = true, bool isPop = false, bool getMastro = false, bool getOnlyMeta = false, bool leggiDaOgniTracciato = false)
        //{
        //    Dictionary<string, object> gruppi_e_singoli_trovati = new Dictionary<string, object>();

        //    HashSet<string> codiciGruppiSet = new HashSet<string>(this.ctx2.PromoTracciatiRecords
        //        .Where(f => f.IdTracciato == idTracciato)
        //        .Select(f => f.CodiceGruppo!));

        //    var tracciato = this.ctx2.PromoTracciatis.Where(f => f.Id == idTracciato).FirstOrDefault();
        //    if (tracciato == null)
        //    {
        //        return Ok(gruppi_e_singoli_trovati);
        //    }

        //    gruppi_e_singoli_trovati.Add("nomeTracciato", tracciato!.Sigla!);
        //    gruppi_e_singoli_trovati.Add("idTracciato", tracciato.Id);
        //    List<string> codiciGruppi = codiciGruppiSet.ToList();
        //    foreach (string gruppo in codiciGruppi)
        //    {
        //        IActionResult? elementi = null;
        //        elementi = await LeggiTracciatoRecord(gruppo, idTracciato, importa, isPop, getMastro, getOnlyMeta, leggiDaOgniTracciato);
        //        if (elementi is OkObjectResult)
        //        {
        //            var okResult = elementi as OkObjectResult;
        //            if (okResult!.Value is List<Dictionary<string, object>>)
        //            {
        //                gruppi_e_singoli_trovati.Add(gruppo, (okResult.Value as List<Dictionary<string, object>>)!);
        //            }
    ...
    .ToList();
                ...
                //if (codiceOrCodiceGruppo == null || codiceOrCodiceGruppo == "" || codiceOrCodiceGruppo == "0" || codiceOrCodiceGruppo == "undefined")
                //{
                //    throw new Exception("Codice non valido");
                //}

                //JObject oDeclMeccaniche = JObject.Parse(System.IO.File.ReadAllText(this.path_external_source + "SourceDeclinazioneMeccaniche.json"));
                //var declMeccDB = oDeclMeccaniche.ToObject<DbDeclinazioneMeccaniche>();
                //JObject oFrame = JObject.Parse(System.IO.File.ReadAllText(this.path_external_source + "SourceFrameworkCss.json"));
                //var frameDB = oFrame.ToObject<DbFrameworkCss>();

                //if (codiceOrCodiceGruppo.Contains(","))
                //{
                //    var tracciato = this.ctx2.PromoTracciatis.Include(f => f.IdImportazioneNavigation).Where(f => f.Id == idTracciato).FirstOrDefault();
                //    if (tracciato == null)
                //    {
                //        throw new Exception("Tracciato non trovato in LeggiTracciatoRecord");
                //    }
                //    var elementiGruppo = this.ctx2.PromoTracciatiRecords.Where(f => f.IdTracciato == idTracciato && codiceOrCodiceGruppo == f.CodiceGruppo).ToList();
                //    if (elementiGruppo.Count == 0)
                //    {
                //        throw new Exception("Gruppo non presente in tracciato");
                //    }

                //    string firmaGruppo = "";
                //    using (var md5 = System.Security.Cryptography.MD5.Create())
                //    {
                //        elementiGruppo = elementiGruppo.OrderBy(f => f.Codice).ToList();
                //        foreach (var rec in elementiGruppo)
                //        {
                //            var dato = IstantaJson.getJsonObject(rec.Dato!);
                //            if (dato.ContainsKey(_key_tracciato_firma))
                //            {
                //                firmaGruppo += dato[_key_tracciato_firma].ToString();
                //            }
                //            else
                //            {
                //                throw new Exception(rec.Codice + " non ha nel Dato la chiave " + _key_tracciato_firma);
                //            }
                //        }
                ...
                //    foreach (var item in elementiGruppo)
                //    {
                //        var el = IstantaJson.getJsonObject(item.Dato!);
                //        if (!getOnlyMeta)
                //        {
                //            bool HasFoto = returnHasFoto(el);
                //            //el[keyXMLSelezione] = item.SelezioneMenabo;
                //            el[keyHasFoto] = HasFoto;

                //            var _artSingolo = this.ctx.Articolis.Where(a => a.Codice == el[keyCodiceRef].ToString()).Select(s => new { Id = s.Id, Codice = s.Codice }).FirstOrDefault();
                //            if (_artSingolo != null)
                //            {
                //                ArticoliFoto? foto = this.ctx.ArticoliFotos.Where(d => d.IdArticolo == _artSingolo.Id && (!d.Tipo.HasValue || d.Tipo == (Byte)TipoFoto.Foto)).OrderByDescending(o => o.DataModifica).FirstOrDefault();
                //                if (foto != null)
                //                {
                //                    //ATTENZIONE: controlliamo che qui non incida sul db realmente
                //                    el[keyNomeFoto] = foto.NomeReale;
                //                    el[keyGuidFoto] = foto.GuidId;
                //                }
                //                else
                //                {
                //                    el[keyNomeFoto] = _artSingolo.Codice + ".psd";
                //                }

                //                if (!el.ContainsKey(_key_foto_extra))
                //                {
                //                    List<ArticoliFoto> af_extra_list = this.ctx.ArticoliFotos.Where(d => d.IdArticolo == _artSingolo.Id && (d.Tipo.HasValue && d.Tipo != (Byte)TipoFoto.Foto)).ToList();
                //                    el[_key_foto_extra] = af_extra_list;
                //                }

                //                el[keyFotoEscluse] = this.ctx.FotoEscluses.Where(d => d.IdArticolo == _artSingolo.Id).ToList();
                //            }

                //            var recordPostMecc = IdentificaMeccanicaRecord(idTracciato, item.Codice!, declMeccDB);
                //            recordPostMecc.record!.recordInTracciato!.Add(GLOBAL_VARIABLES.combinazioneMeccanica, recordPostMecc.meccanicheValide);
                //            recordPostMecc.record.recordInTracciato.Add(GLOBAL_VARIABLES.combinazioneAssegnata, recordPostMecc.meccanicaAssegnata);
                //            recordPostMecc.record.recordInTracciato.Add(GLOBAL_VARIABLES.meccanicaInvalidata, recordPostMecc.meccanicaInvalidata);
                //            el.Add(GLOBAL_VARIABLES.combinazioneMeccanica, recordPostMecc.meccanicheValide);
                //            el.Add(GLOBAL_VARIABLES.combinazioneAssegnata, recordPostMecc.meccanicaAssegnata);
                //            el.Add(GLOBAL_VARIABLES.meccanicaInvalidata, recordPostMecc.meccanicaInvalidata);
                //            var codiceBox = IdentificaCodiceBox(recordPostMecc.record, frameDB);
                //            el.Add(GLOBAL_VARIABLES_FICO.codiceBox, codiceBox);

                //            if (importa)
                //            {
                //                List<Dictionary<string, object>> listaTEMP = new List<Dictionary<string, object>>();

                //                AddestramentoExcel? addestramento = this.ctx2.AddestramentoExcels.Find(tracciato.IdImportazioneNavigation.IdAddestramento);
                //                if (addestramento!.externalCallPerExport != null)
                //                {
                //                    el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr1Tracciato] = el.ContainsKey(descr1Key) ? el[descr1Key] : "";
                //                    el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr2Tracciato] = el.ContainsKey(descr2Key) ? el[descr2Key] : "";
                //                    el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr3Tracciato] = el.ContainsKey(descr3Key) ? el[descr3Key] : "";
                //                    el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr4Tracciato] = el.ContainsKey(descr4Key) ? el[descr4Key] : "";

                //                    Articoli art = new Articoli();
                //                    //singolo
                //                    var _art = this.ctx.Articolis.Where(a => a.Codice == el[keyCodiceRef].ToString()).Select(s => new { Id = s.Id, Codice = s.Codice }).FirstOrDefault();//cod_refs_in_tracciato.Contains(a.Codice))
                ...
                //                        ArticoliDescrizioni? descr = this.ctx.ArticoliDescrizionis.Where(d => d.IdArticolo == _art.Id && d.Area == tracciato.Area && d.Canale == tracciato.Canale).FirstOrDefault();
                //                        if (descr != null)
                //                        {
                ...
                //                        else
                //                        {
                //                            descr = this.ctx.ArticoliDescrizionis.Where(d => d.IdArticolo == _art.Id && d.Area == tracciato.Area).FirstOrDefault();
                //                            if (descr != null)
                //                            {
                //                                art.ArticoliDescrizionis!.Add(descr);
                //                            }
                //                            else
                //                            {
                //                                el[keyDescrizioneCanale] = false;
                //                                el[keyDescrizioneRegionale] = false;
                //                                descr = this.ctx.ArticoliDescrizionis.Where(d => d.IdArticolo == _art.Id).FirstOrDefault();
                //                                if (descr != null)
                //                                {
                //                                    art.ArticoliDescrizionis!.Add(descr);
                //                                }
                ...
                //                    ArticoliDescrizioni? artDescr = art.ArticoliDescrizionis!.OrderByDescending(o => o.DataUltimaRicezione).FirstOrDefault();
                //                    if (artDescr != null)
                //                    {
                //                        if (artDescr.FirmaTracciato == null || artDescr.FirmaTracciato == "")
                //                        {
                //                            el[keyStatoRevisioneSingolo] = statoRevisione.DaRevisionare;
                //                        }
                //                        else
                //                        {
                //                            var dato = IstantaJson.getJsonObject(item!.Dato!);
                //                            if (dato.ContainsKey(_key_tracciato_firma))
                //                            {
                //                                if (dato[_key_tracciato_firma].ToString() == artDescr.FirmaTracciato)
                //                                {
                //                                    el[keyStatoRevisioneSingolo] = statoRevisione.Revisionato;
                //                                }
                //                                else
                //                                {
                //                                    el[keyStatoRevisioneSingolo] = statoRevisione.DaConfermare;
                //                                }
                //                            }
                //                            else
                //                            {
                //                                throw new Exception(item.Codice + " non contiene nel suo Dato la chiave " + _key_tracciato_firma + ", impossibile declamare lo stato di revisione");
                //                            }
                //                        }

                //                        el[descr1Key] = artDescr.Descrizione1!;
                //                        el[descr2Key] = artDescr.Descrizione2!;
                //                        el[descr3Key] = artDescr.Descrizione3!;
                //                        el[descr4Key] = artDescr.Descrizione4!;
                //                        el[keyDataUltimaRicezione] = artDescr.DataUltimaRicezione;
                //                        if (artDescr.DescrizioneIndd != null)
                //                            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrIndd] = artDescr.DescrizioneIndd;
                //                        if (artDescr.Peso != null)
                //                            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrPeso] = artDescr.Peso.Value;
                //                        if (artDescr.Um != null)
                //                            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrUm] = artDescr.Um;
                //                        if (artDescr.Extra != null)
                //                            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrExtra] = JsonConvert.DeserializeObject<JObject>(artDescr.Extra)!;
                //                    }
                //                    else
                //                    {
                //                        el[keyStatoRevisioneSingolo] = statoRevisione.DaRevisionare;
                ...
                //                    if (el[key_codice_gruppo].ToString() != el[keyCodiceRef].ToString())
                //                    {

                //                        //gruppo
                //                        ArticoliDescrizioni? descrGruppo = this.ctx.ArticoliDescrizionis.Where(a => a.CodiceGruppo == el[key_codice_gruppo].ToString() && a.Area == tracciato.Area && a.Canale == tracciato.Canale).FirstOrDefault();

                //                        if (descrGruppo == null)
                //                        {
                //                            descrGruppo = this.ctx.ArticoliDescrizionis.Where(a => a.CodiceGruppo == el[key_codice_gruppo].ToString() && a.Area == tracciato.Area).FirstOrDefault();
                //                            if (descrGruppo == null)
                //                            {
                //                                el[keyDescrizioneCanale] = false;
                //                                el[keyDescrizioneRegionale] = false;
                //                                descrGruppo = this.ctx.ArticoliDescrizionis.Where(a => a.CodiceGruppo == el[key_codice_gruppo].ToString()).FirstOrDefault();
                //                            }
                //                            else
                //                            {
                //                                el[keyDescrizioneCanale] = false;
                //                                el[keyDescrizioneRegionale] = true;
                //                            }
                //                        }
                //                        else
                //                        {
                //                            el[keyDescrizioneCanale] = true;
                //                            el[keyDescrizioneRegionale] = true;
                //                        }

                //                        if (descrGruppo != null)
                //                        {
                //                            if (descrGruppo.FirmaTracciato == null || descrGruppo.FirmaTracciato == "")
                //                            {
                //                                el[keyStatoRevisioneGruppo] = statoRevisione.DaRevisionare;
                //                            }
                //                            else if (descrGruppo.FirmaTracciato == firmaGruppo)
                //                            {
                //                                el[keyStatoRevisioneGruppo] = statoRevisione.Revisionato;
                //                            }
                //                            else
                //                            {
                //                                el[keyStatoRevisioneGruppo] = statoRevisione.DaConfermare;
                //                            }
                //                            Dictionary<string, object> dictDescrGruppo = new Dictionary<string, object>();
                //                            dictDescrGruppo.Add(descr1Key, SessionIstantaObject.ConvertACapoConBrTagPerXml(descrGruppo.Descrizione1!));
                //                            dictDescrGruppo.Add(descr2Key, SessionIstantaObject.ConvertACapoConBrTagPerXml(descrGruppo.Descrizione2!));
                //                            dictDescrGruppo.Add(descr3Key, SessionIstantaObject.ConvertACapoConBrTagPerXml(descrGruppo.Descrizione3!));
                //                            dictDescrGruppo.Add(descr4Key, SessionIstantaObject.ConvertACapoConBrTagPerXml(descrGruppo.Descrizione4!));
                //                            dictDescrGruppo.Add(keyDataUltimaRicezione, descrGruppo.DataUltimaRicezione);
                //                            //el[descr1Key] = descrGruppo.Descrizione1;
                //                            //el[descr2Key] = descrGruppo.Descrizione2;
                //                            //el[descr3Key] = descrGruppo.Descrizione3;
                //                            //el[descr4Key] = descrGruppo.Descrizione4;
                //                            //el[keyDataUltimaRicezione] = descrGruppo.DataUltimaRicezione;
                ...
                //                                //el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrIndd] = descrGruppo.DescrizioneIndd;
                //                            }
                //                            else if (el.ContainsKey(Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrIndd))
                //                            {
                //                                //el.Remove(Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrIndd);
                //                            }
                ...
                //                                //el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrPeso] = descrGruppo.Peso;
                //                            }
                ...
                //                                //el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrUm] = descrGruppo.Um;
                //                            }
                ...
                //                                //el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrExtra] = descrGruppo.Extra;
                //                            }

                //                            el[GLOBAL_VARIABLES.keyXMLDescrizioneGruppo] = dictDescrGruppo;
                //                        }
                //                        else
                //                        {
                //                            el[keyStatoRevisioneGruppo] = statoRevisione.DaRevisionare;
                //                            Dictionary<string, object> dictDescrGruppo = new Dictionary<string, object>();
                //                            dictDescrGruppo.Add(descr1Key, "");
                //                            dictDescrGruppo.Add(descr2Key, "");
                //                            dictDescrGruppo.Add(descr3Key, "");
                //                            dictDescrGruppo.Add(descr4Key, "");
                //                            //dictDescrGruppo.Add(keyDataUltimaRicezione, null);
                //                            el[GLOBAL_VARIABLES.keyXMLDescrizioneGruppo] = dictDescrGruppo;
                //                        }
                //                    }
                //                    Dictionary<string, object> tmpDict = new Dictionary<string, object>();
                //                    tmpDict.Add("refs", new List<Dictionary<string, object>>() { el });
                //                    listaTEMP.Add(tmpDict);
                //                }


                //                Dictionary<string, string> paramsToExternal = new Dictionary<string, string>();
                //                paramsToExternal["importFieldsStringfy"] = tracciato.IdImportazioneNavigation.ParamsRequest!;
                //                paramsToExternal["tracciatoFieldsStringfy"] = tracciato.Meta!;
                //                paramsToExternal["esportaDaArchivio"] = "true";
                //                paramsToExternal["esportaDaMenabo"] = "true";
                //                paramsToExternal["esportaFoto"] = "true";
                //                paramsToExternal["Titolo"] = tracciato.Sigla!;

                //                Dictionary<string, object> _pass = new Dictionary<string, object>();
                //                _pass["tracciato"] = listaTEMP;
                //                _pass["formRequest"] = paramsToExternal;
                ...
                //                if (parsingRes != null && parsingRes.liste![0].Records.Count > 0)
                //                {
                //                    foreach (var records in parsingRes.liste[0].Records)
                //                    {
                //                        //JArray jA = item["refs"] as JArray;
                //                        //var listaRefs =  jA.ToList(List<Dictionary<string, object>>);
                //                        var jArr = records["refs"] as JArray;
                //                        var listaRefs = jArr!.ToObject<List<Dictionary<string, object>>>();
                //                        var itemInRefs = listaRefs![0];
                //                        if (itemInRefs.ContainsKey(keyFotoExtraAuto))
                //                        {
                //                            var listaFotoExtraAuto = (itemInRefs[keyFotoExtraAuto] as JArray)!.ToObject<List<FotoExtraAutoItem>>();
                //                            foreach (var fotoExtraAuto in listaFotoExtraAuto!)
                //                            {
                //                                if (((itemInRefs[keyFotoEscluse] as JArray)!.ToObject<List<FotoEscluse>>())!.Find(f => f.NomeReale == fotoExtraAuto.NomeFoto) != null)
                //                                {
                //                                    fotoExtraAuto.Escluso = true;
                //                                }
                //                            }
                //                            itemInRefs[keyFotoExtraAuto] = listaFotoExtraAuto;
                //                        }
                //                        else
                //                        {
                //                            itemInRefs[keyFotoExtraAuto] = new List<FotoExtraAutoItem>();
                //                        }

                //                        if (getMastro)
                //                        {
                //                            var articolo = IdentificaArticoloInRevisione(idTracciato, el[keyCodiceRef].ToString()!);
                //                            var recordConMastro = GetMastroAssociate(articolo);
                //                            itemInRefs[keyRegoleMastro] = recordConMastro.regoleMastro;
                //                            itemInRefs[keyAllEtichette] = articolo.allEtichette!;
                //                            itemInRefs[keyEtichetteVisual] = articolo.etichetteVisual!;
                //                        }
                //                        else
                //                        {
                //                            var articolo = IdentificaArticoloInRevisione(idTracciato, el[keyCodiceRef].ToString()!);
                ...
                //                        elementiTrovati.Add(itemInRefs);
                //                    }
                ...
                //                    var articolo = IdentificaArticoloInRevisione(idTracciato, el[keyCodiceRef].ToString()!);
                //                    var recordConMastro = GetMastroAssociate(articolo);
                //                    el[keyRegoleMastro] = recordConMastro.regoleMastro;
                //                    el[keyAllEtichette] = articolo.allEtichette!;
                //                    el[keyEtichetteVisual] = articolo.etichetteVisual!;
                //                }
                //                else
                //                {
                //                    var articolo = IdentificaArticoloInRevisione(idTracciato, el[keyCodiceRef].ToString()!);
                ...
                //    var element = this.ctx2.PromoTracciatiRecords.Where(f => f.IdTracciato == idTracciato && codiceOrCodiceGruppo == f.Codice).FirstOrDefault();
                //    if (element == null)
                //    {
                //        throw new Exception("Record non presente in tracciato");
                //    }
                //    var el = IstantaJson.getJsonObject(element.Dato!);
                //    if (!getOnlyMeta)
                //    {
                //        var recordPostMecc = IdentificaMeccanicaRecord(idTracciato, element.Codice!, declMeccDB);
                //        recordPostMecc.record!.recordInTracciato!.Add(GLOBAL_VARIABLES.combinazioneMeccanica, recordPostMecc.meccanicheValide);
                //        recordPostMecc.record.recordInTracciato.Add(GLOBAL_VARIABLES.combinazioneAssegnata, recordPostMecc.meccanicaAssegnata);
                //        recordPostMecc.record.recordInTracciato.Add(GLOBAL_VARIABLES.meccanicaInvalidata, recordPostMecc.meccanicaInvalidata);
                //        el.Add(GLOBAL_VARIABLES.combinazioneMeccanica, recordPostMecc.meccanicheValide);
                ...
                //        if (importa)
                //        {
                //            //el[keyXMLSelezione] = element.SelezioneMenabo;
                //            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr1Tracciato] = el.ContainsKey(descr1Key) ? el[descr1Key] : "";
                //            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr2Tracciato] = el.ContainsKey(descr2Key) ? el[descr2Key] : "";
                //            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr3Tracciato] = el.ContainsKey(descr3Key) ? el[descr3Key] : "";
                //            el[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr4Tracciato] = el.ContainsKey(descr4Key) ? el[descr4Key] : "";
                ...
                //                List<ArticoliFoto> fotos = this.ctx.ArticoliFotos.Where(d => d.IdArticolo == _art.Id).OrderByDescending(o => o.DataModifica).ToList();
                //                art.ArticoliFotos = fotos;
                //            }
                ...
                //                else
                //                {
                //                    var dato = IstantaJson.getJsonObject(element.Dato!);
                //                    if (dato.ContainsKey(_key_tracciato_firma))
                //                    {
                //                        if (dato[_key_tracciato_firma].ToString() == artDescr.FirmaTracciato)
                //                        {
                ...
                //                    else
                //                    {
                //                        throw new Exception(element.Codice + " non contiene nel suo Dato la chiave " + _key_tracciato_firma + ", impossibile declamare lo stato di revisione");
                //                    }
                //                }
                ...
                //            if (art != null)
                //            {
                //                if (!el.ContainsKey(keyNomeFoto))
                //                {

                //                    ArticoliFoto? af = art.ArticoliFotos!.Where(f => !f.Tipo.HasValue || f.Tipo == (Byte)TipoFoto.Foto).OrderByDescending(o => o.DataModifica).FirstOrDefault();//.Where(af => af.StatoSelezione == (Byte)StatoSelezioneFoto.Primaria).OrderByDescending(o => o.DataModifica).ThenByDescending(o2 => o2.DataInserimento).FirstOrDefault();
                //                    if (af != null)
                //                    {
                //                        //ATTENZIONE: controlliamo che qui non incida sul db realmente
                //                        el[keyNomeFoto] = af.NomeReale;
                //                        el[keyGuidFoto] = af.GuidId;
                //                    }
                //                    else
                //                    {
                //                        el[keyNomeFoto] = art.Codice + ".psd";
                //                    }
                //                }

                //                if (!el.ContainsKey(_key_foto_extra))
                //                {
                //                    List<ArticoliFoto> af_extra_list = this.ctx.ArticoliFotos.Where(d => d.IdArticolo == _art!.Id && (d.Tipo.HasValue && d.Tipo != (Byte)TipoFoto.Foto)).ToList();
                //                    el[_key_foto_extra] = af_extra_list;
                //                }

                //                el[keyFotoEscluse] = this.ctx.FotoEscluses.Where(d => d.IdArticolo == _art!.Id).ToList();
                ...
                //            _pass["tracciato"] = listaTEMP;
                //            _pass["formRequest"] = paramsToExternal;
                //            if (isPop ? addestramento!.externalCallPerExportPoP != null : addestramento!.externalCallPerExport != null)
                //            {
                //                string resultExternal = icItem.execLibFunction(isPop ? addestramento.externalCallPerExportPoP! : addestramento.externalCallPerExport!, _pass).ToString()!;
                ...
                //                if (parsingRes != null && parsingRes.liste![0].Records.Count > 0)
                //                {
                //                    foreach (var item in parsingRes.liste[0].Records)
                //                    {
                //                        //JArray jA = item["refs"] as JArray;
                //                        //var listaRefs =  jA.ToList(List<Dictionary<string, object>>);
                ...
                //            //singolo
                //            var _art = this.ctx.Articolis.Where(a => a.Codice == el[keyCodiceRef].ToString()).Select(s => new { Id = s.Id, Codice = s.Codice }).FirstOrDefault();
    ...
    .Max(r => r.Versione);
        ...
        //public async Task<IActionResult> getSchedeRefs([FromForm] string codiciGruppo, int idLavorazione, bool byPassLavorazioneRecord = false, FicoCombinazioneKitReadMode mode = FicoCombinazioneKitReadMode.Classic)
        //{
        //    try
        //    {
        //        if(codiciGruppo == null)
        //        {
        //            codiciGruppo = "";
        //        }
        //        List<string> ListaCodici = codiciGruppo.Split("-").ToList();
        //        List <ArticoloInRevisioneKitResult> resultList = new List<ArticoloInRevisioneKitResult>();
        //        foreach (var cod in ListaCodici)
        //        {
        //            IActionResult res = await getSchedaRef(cod, idLavorazione, byPassLavorazioneRecord);
        ...
        //                    {
        //                        records = new List<ArticoloInKit>(),
        //                        error = (res as OkObjectResult)!.Value as string,
        //                        esito = false,
        //                    });
        //                }
        //                else
...
.Select(f => f.Versione).ToList();
        ...
        [HttpGet]
        [Route("Menabo/GetSchedaRecordTracciato/{idRec}/{mode}")]
        public async Task<IActionResult> GetSchedaRecordTracciato(int idRec, FicoCombinazioneKitReadMode mode)
        {
            //try
            //{
            //    if (ficoController == null)
            //    {
            //        ficoController = new FicoProcessController(_config, _external_lib, this._fico_conf, _option_import, this.httpClient, _cache);
            //    }

            //    //PromoLavorazioni plItem = this.ctx2.PromoLavorazionis.FirstOrDefault(p => p.Id == idLavorazione);
            //    ficoController.HttpContextInRent = HttpContext;
            //    //var promise = await ficoController.processaKit(idLavorazione, FicoCombinazioneKitReadMode.Classic);
            //    var okResult = promise as OkObjectResult;
            //    result = (okResult.Value as ArticoloInRevisioneKitResult);
            //}
            //catch (Exception ex)
...
.ToList();
    ...
    int idOperazione)
        ...
        //[HttpPut]
        //[Route("Menabo/modificaPagina/{idOperazione}")]
        //public async Task<IActionResult> modificaPaginaDellaReferenza(InddObjCambioPaginaRequest dato, int idOperazione)
        //{
        //    BoolResult result = new BoolResult();
        //    try
        //    {
        ...
        //        PromoLavorazioniRecord? plrItem = GetLavorazioneRecordConFratelli(
        //            dato.CodiceGruppo!,
        //            dato.idRec,
        //            dato.idLavorazione,
        //            out recordTarget
        ...
        //        plrItem.Pagina = dato.pag;
        //        this.ctx2.SaveChanges();

        //        registraAttivitaMenabo(idOperazione, dato.CodiceGruppo!, "modificaPagina", tipoOperazione.cambioPagina, null, plrItem.Id);
        ...
        [Route("Menabo/getFileByte")]
        public async Task<IActionResult> getFileByte(FileTest obj)
        {
            try
            {
                byte[] arrBytes = obj.byteList!.ToArray();

                using (System.IO.FileStream fs = new System.IO.FileStream(path_export + System.IO.Path.DirectorySeparatorChar + obj.nomeFile, FileMode.Create))
                {
                    fs.Write(arrBytes, 0, arrBytes.Length);
                }

                return Ok(arrBytes.Length);
            }
            catch (Exception ex)
            {
                return Ok(ex.ToString());
    ...
    gruppi.ListaCodiciConId != null && gruppi.ListaCodiciConId.Count > 0
        ...
        : (gruppi.ListaCodici ?? new List<string>())
            ...
            .ToList();
                ...
                //foreach (var gruppo in gruppi.ListaCodici)
                //{
                //    var lavoriazioneRecord = this.ctx2.PromoLavorazioniRecords.Include(i=>i.IdPromoLavorazioniNavigation).Where(f => f.CodiceGruppo == gruppo && idLavorazione == f.IdLavorazione).FirstOrDefault();
                //    if (lavoriazioneRecord == null)
                //    {
                ...
                //    if (kit==null)
                //        kit = JsonConvert.DeserializeObject<FicoRuntimeKit>(lavoriazioneRecord.IdPromoLavorazioniNavigation.Meta!);
                ...
                //    {
                //        throw new Exception("L'elemento da cui ereditare la pagina risulta trovarsi alla pagina non valida: " + paginaPrimoElemento+". Operazione bloccata, contattare un tecnico.");
                //    }
                ...
                //        }
                //    }
                //    var recordsGruppo = this.ctx2.PromoTracciatiRecords.Where(f => f.CodiceGruppo == gruppo && f.IdTracciato == idTracciato).ToList();
                ...
                if (op is OkObjectResult && (op as OkObjectResult)!.Value is CambioMetaRecordTracciatoResponse)
                {
                    ...
                    else
                    {
                        ...
                        if(schedaRefResult.records.Count == 0)
                        {
                            ...
                            rimuoviRefImpaginata(idLavorazione, false, gruppoDaRimuovere);
                            result.item = null;
                            return Ok(result);
                        }
...
#warning VULNERABILITA'. Quello che avviene adesso sul controllo della chiave CORE Descirizoni per il processo di cambio strutturale NON è corretto e potrebbe portare a dele brutte sorprese. Quando torneremo a parlare DA PLUGIN del tratamento delle descrizioni regionali, dovrmeo fare un pensiero sull'esporre i parametri PESO e UM al pari dei campi descrittivi 1,2,3,4
        ...
        //public async Task<IActionResult> rimuoviRefImpaginata(int idLavorazione, bool eliminaDaTracciato, ListaGruppiRaggruppamento codiciGruppo)
        //{
        //    BoolResult result = new BoolResult();
        //    try
        //    {
        ...
        //        if (codiciGruppo.ListaCodici != null)
        //        {
        ...
        //            if (ficoController == null)
        //            {
        //                ficoController = new FicoProcessController(_config, _external_lib, this._fico_conf, _option_import, this.httpClient, _cache, this._dbContextFactory, null);
        //            }


        //            var tracciati = ficoController.getTracciatiFromIdkitLavorazione(idLavorazione);

        //            foreach (var codiceGruppo in codiciGruppo.ListaCodici)
        //            {
        //                var refImpaginata = this.ctx2.PromoLavorazioniRecords.Where(f => f.CodiceGruppo == codiceGruppo && f.IdLavorazione == idLavorazione).FirstOrDefault();
        //                if (refImpaginata == null)
        //                {
        //                    result.error += "L'elemento " + codiceGruppo + " non era già presente nell'impaginato ";
        //                }
        //                else
        //                {
        ...
        //                    var records = this.ctx2.PromoTracciatiRecords.Where(f => f.IdTracciato == idTracciato && f.CodiceGruppo == codiceGruppo).ToList();
    ...
    ListaGruppiRaggruppamento codiciGruppo)
        ...
        [HttpPut]
        [Route("Menabo/impaginaRecordSingolo/{idTracciato}/{meccanica}/{pageNumber}")]
        public async Task<IActionResult> impaginaRecordSingolo(string codiceGruppo, int idTracciato, string meccanica, int pageNumber)
        {
            BoolResult result = new BoolResult();
            MenaboRef refToAdd = new MenaboRef();
            try
            {
                bool alreadyImpaginata = false;

                bool isGruppo = codiceGruppo.Split(",").Length > 1;
                var element = this.ctx2.PromoTracciatiRecords.Where(f => f.IdTracciato == idTracciato && (isGruppo ? f.CodiceGruppo == codiceGruppo : f.Codice == codiceGruppo)).FirstOrDefault();
                if (element == null)
                {
                    throw new Exception((isGruppo ? "gruppo" : "Record") + " non trovato in promoTracciatiRecord");
                }
                refToAdd = this.ctx2.MenaboRefs.Include(f => f.IdPaginaNavigation).Where(f => (!isGruppo ? f.IdRecord == element.Id : f.CodiceGruppo == codiceGruppo) && f.IdPaginaNavigation.IdTracciato == idTracciato).FirstOrDefault()!;
                if (refToAdd == null)
                {
                    refToAdd = new MenaboRef();
                }
                else
                {
                    alreadyImpaginata = true;
                }

                if (!isGruppo)
                {
                    refToAdd.CodiceGruppo = null;
                    refToAdd.IdRecord = element.Id;
                }
                else
                {
                    refToAdd.CodiceGruppo = codiceGruppo;
                    refToAdd.IdRecord = null;
                }

                refToAdd.Formato = "1x1";
                List<CombinazioniMeccaniche> listMecc = new List<CombinazioniMeccaniche>();
                CombinazioniMeccaniche mecc = new CombinazioniMeccaniche();
                mecc.Id = 0;
                mecc.NomeCombinazione = meccanica;
                mecc.Formato = "1x1";
                listMecc.Add(mecc);
                refToAdd.Meccaniche = JsonConvert.SerializeObject(listMecc);

                var page = this.ctx2.MenaboPagines.Where(f => f.IdTracciato == idTracciato && f.Numero == pageNumber).FirstOrDefault();
                if (page == null)
                {
                    page = new MenaboPagine();
                    page.Formato = "1x1";
                    page.Numero = (short)pageNumber;
                    page.IdTracciato = idTracciato;
                    page.IdMastro = 1;
                    this.ctx2.MenaboPagines.Add(page);
                    this.ctx2.SaveChanges();
                    page = this.ctx2.MenaboPagines.Where(f => f.IdTracciato == idTracciato && f.Numero == pageNumber).FirstOrDefault();
                }
                refToAdd.IdPagina = page!.Id;
                refToAdd.Indice = 999;
                if (!alreadyImpaginata)
                {
                    this.ctx2.MenaboRefs.Add(refToAdd);
                }
                this.ctx2.SaveChanges();
        ...
        [HttpGet]
        [Route("Menabo/impaginaPopFromIndd/{idTracciato}")]
        public async Task<IActionResult> impaginaPopFromIndd(int idTracciato)
        {
            try
            {
                IstantaController icItem = new IstantaController(this._config.GetConnectionString("IstandaConnectionDb")!, this.path_external_lib, this.path_external_source, this._dbContextFactory);
                ...
                PromoTracciati? tracciato = this.ctx2.PromoTracciatis.Include(f => f.PromoTracciatiRecords).Include(f => f.IdImportazioneNavigation).Where(f => f.Id == idTracciato).FirstOrDefault();
                List<PromoTracciatiRecord> q_records = tracciato!.PromoTracciatiRecords.Where(f => f.Stato == (byte)StatoRecord.Attivo).ToList();
                ...
                List<Dictionary<string, object>> tracciatiSingoliDict = q_records.Select(s => IstantaJson.getJsonObject(s.Dato!)).ToList();

                tracciatiSingoliDict = Ordinamento.ordinaRecordsTracciato(tracciatiSingoliDict, exClass, icCtrl, this.path_external_source, this._fico_conf.Value.nomeCliente);
                ...
                if (addestramento!.externalCallPerExport != null)
                {
                    ...
                    List<Dictionary<string, object>> listaTEMP = new List<Dictionary<string, object>>();
                    Dictionary<string, string> md5GruppiIndividuati = new Dictionary<string, string>();
                    foreach (var element in tracciatiSingoliDict)
                    {
                        element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr1Tracciato] = element.ContainsKey(descr1Key) ? element[descr1Key] : "";
                        element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr2Tracciato] = element.ContainsKey(descr2Key) ? element[descr2Key] : "";
                        element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr3Tracciato] = element.ContainsKey(descr3Key) ? element[descr3Key] : "";
                        element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr4Tracciato] = element.ContainsKey(descr4Key) ? element[descr4Key] : "";

                        Articoli art = new Articoli();


                        //singolo
                        var _art = this.ctx.Articolis.Where(a => a.Codice == element[keyCodiceRef].ToString()).Select(s => new { Id = s.Id, Codice = s.Codice }).FirstOrDefault();//cod_refs_in_tracciato.Contains(a.Codice))
                        ...
                        if (_art != null)
                        {
                            ...
                            if (descr != null)
                            {
                                element[keyDescrizioneCanale] = true;
                                element[keyDescrizioneRegionale] = true;
                                art.ArticoliDescrizionis!.Add(descr);
                            }
                            else
                            {
                                ...
                                if (descr != null)
                                {
                                    element[keyDescrizioneCanale] = false;
                                    element[keyDescrizioneRegionale] = true;
                                    art.ArticoliDescrizionis!.Add(descr);
                                }
                                else
                                {
                                    descr = this.ctx.ArticoliDescrizionis.Where(d => d.IdArticolo == _art.Id).FirstOrDefault();
                                    element[keyDescrizioneCanale] = false;
                                    element[keyDescrizioneRegionale] = false;
                                    if (descr != null)
                                    {
                                        art.ArticoliDescrizionis!.Add(descr);
                                    }
                        ...
                        if (artDescr != null)
                        {
                            ...
                            if (artDescr.FirmaTracciato == null || artDescr.FirmaTracciato == "")
                            {
                                element[keyStatoRevisioneSingolo] = statoRevisione.DaRevisionare;
                            }
                            else
                            {
                                var dato = element;
                                if (dato.ContainsKey(_key_tracciato_firma))
                                {
                                    if (dato[_key_tracciato_firma].ToString() == artDescr.FirmaTracciato)
                                    {
                                        element[keyStatoRevisioneSingolo] = statoRevisione.Revisionato;
                                    }
                                    else
                                    {
                                        element[keyStatoRevisioneSingolo] = statoRevisione.DaConfermare;
                                    }
                                }
                                else
                                {
                                    throw new Exception(element[keyCodiceRef] + " non contiene nel suo Dato la chiave " + _key_tracciato_firma + ", impossibile declamare lo stato di revisione");
                                }
                            }

                            element[descr1Key] = artDescr.Descrizione1!;
                            element[descr2Key] = artDescr.Descrizione2!;
                            element[descr3Key] = artDescr.Descrizione3!;
                            element[descr4Key] = artDescr.Descrizione4!;
                            if (artDescr.DescrizioneIndd != null)
                                element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrIndd] = artDescr.DescrizioneIndd;
                            if (artDescr.Peso != null)
                                element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrPeso] = artDescr.Peso.Value;
                            if (artDescr.Um != null)
                                element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrUm] = artDescr.Um;
                            if (artDescr.Extra != null)
                                element[Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrExtra] = JsonConvert.DeserializeObject<JObject>(artDescr.Extra!)!;
                        }
                        else
                        {
                            element[keyStatoRevisioneSingolo] = statoRevisione.DaRevisionare;
                        ...
                        if (art != null)
                        {
                            ...
                            if (!element.ContainsKey(_key_foto))
                            {
                                ...
                                {
                                    //ATTENZIONE: controlliamo che qui non incida sul db realmente
                                    element[_key_foto] = af.NomeReale;
                                }
                                else
                                {
                                    element[_key_foto] = art.Codice + ".psd";
                                }
                            }

                            if (!element.ContainsKey(_key_foto_extra))
                            {
                                List<ArticoliFoto> af_extra_list = this.ctx.ArticoliFotos.Where(d => d.IdArticolo == _art!.Id && (d.Tipo.HasValue && d.Tipo != (Byte)TipoFoto.Foto)).ToList();
                                element[_key_foto_extra] = af_extra_list;
                            }

                            element[keyFotoEscluse] = this.ctx.FotoEscluses.Where(d => d.IdArticolo == _art!.Id).ToList();

                        }
                        if (element[key_codice_gruppo].ToString()!.Contains("7810803"))
                        {
                            Debug.WriteLine("");
                        }

                        if (element[key_codice_gruppo].ToString() != element[keyCodiceRef].ToString())
                        {
                            //gruppo
                            string firmaGruppo = "";
                            if (!md5GruppiIndividuati.ContainsKey(element[key_codice_gruppo].ToString()!))
                            {
                                using (var md5 = System.Security.Cryptography.MD5.Create())
                                {
                                    var codici = element[key_codice_gruppo].ToString()!.Split(",");
                                    foreach (var cod in codici)
                                    {
                                        var dictSingolo = tracciatiSingoliDict.Where(f => f.ContainsKey(keyCodiceRef) && f[keyCodiceRef].ToString() == cod).FirstOrDefault();
                                        if (dictSingolo != null && dictSingolo.ContainsKey(_key_tracciato_firma))
                                        {
                                            firmaGruppo += dictSingolo[_key_tracciato_firma].ToString();
                                        }
                                        else
                                        {
                                    ...
                                    byte[] bString = System.Text.ASCIIEncoding.UTF8.GetBytes(firmaGruppo);
                                    firmaGruppo = BitConverter.ToString(md5.ComputeHash(bString)).Replace("-", string.Empty);
                                    md5GruppiIndividuati[element[key_codice_gruppo].ToString()!] = firmaGruppo;
                                }
                            }
                            else
                            {
                                firmaGruppo = md5GruppiIndividuati[element[key_codice_gruppo].ToString()!];
                            }

                            ArticoliDescrizioni? descrGruppo = this.ctx.ArticoliDescrizionis.Where(a => a.CodiceGruppo == element[key_codice_gruppo].ToString() && a.Area == tracciato.Area && a.Canale == tracciato.Canale).FirstOrDefault();

                            if (descrGruppo == null)
                            {
                                descrGruppo = this.ctx.ArticoliDescrizionis.Where(a => a.CodiceGruppo == element[key_codice_gruppo].ToString() && a.Area == tracciato.Area).FirstOrDefault();
                                if (descrGruppo == null)
                                {
                                    element[keyDescrizioneCanale] = false;
                                    element[keyDescrizioneRegionale] = false;
                                    descrGruppo = this.ctx.ArticoliDescrizionis.Where(a => a.CodiceGruppo == element[key_codice_gruppo].ToString()).FirstOrDefault();
                                }
                                else
                                {
                                    element[keyDescrizioneCanale] = false;
                            ...
                            if (descrGruppo != null)
                            {
                                if (descrGruppo.FirmaTracciato == null || descrGruppo.FirmaTracciato == "")
                                {
                                    element[keyStatoRevisioneGruppo] = statoRevisione.DaRevisionare;
                                }
                                else if (descrGruppo.FirmaTracciato == firmaGruppo)
                                {
                                    element[keyStatoRevisioneGruppo] = statoRevisione.Revisionato;
                                }
                                else
                                {
                                    element[keyStatoRevisioneGruppo] = statoRevisione.DaConfermare;
                                }
                                Dictionary<string, object> dictDescrGruppo = new Dictionary<string, object>();
                                dictDescrGruppo.Add(descr1Key, SessionIstantaObject.ConvertACapoConBrTagPerXml(descrGruppo.Descrizione1!));
                                dictDescrGruppo.Add(descr2Key, SessionIstantaObject.ConvertACapoConBrTagPerXml(descrGruppo.Descrizione2!));
                                ...
                                element[GLOBAL_VARIABLES.keyXMLDescrizioneGruppo] = dictDescrGruppo;
                            }
                        }
                        Dictionary<string, object> tmpDict = new Dictionary<string, object>();
                        tmpDict.Add("refs", new List<Dictionary<string, object>>() { element });
                        listaTEMP.Add(tmpDict);
                    }
                    ...
                    if (parsingRes != null && parsingRes.liste![0].Records.Count > 0)
                    {
                        foreach (var itemInRefs in parsingRes.liste[0].Records)
                        {
                            var listaFotoExtraAuto = (itemInRefs[keyFotoExtraAuto] as JArray)!.ToObject<List<FotoExtraAutoItem>>();
                            foreach (var fotoExtraAuto in listaFotoExtraAuto!)
                            {
                                if (((itemInRefs[keyFotoEscluse] as JArray)!.ToObject<List<FotoEscluse>>())!.Find(f => f.NomeReale == fotoExtraAuto.NomeFoto) != null)
                                {
                                    Console.WriteLine($"MB IMP POP 15492 - Escludo foto {fotoExtraAuto.NomeFoto}");
                                    fotoExtraAuto.Escluso = true;
                                }
                            }
                            itemInRefs[keyFotoExtraAuto] = listaFotoExtraAuto;
                            ...
                            var articolo = IdentificaArticoloInRevisione(idTracciato, itemInRefs[keyCodiceRef].ToString()!);
                            var recordConMastro = GetMastroAssociate(articolo);
                            itemInRefs[keyRegoleMastro] = recordConMastro.regoleMastro;
                        }
                        tracciatiSingoliDict = parsingRes.liste[0].Records;
                    }
                    else
                    {
                        throw new Exception("");
                ...
                return Ok(tracciatiSingoliDict);
            }
            catch (Exception ex)
            {
                return Ok(ex.ToString());
        ...
        //[HttpPut]
        //[Route("Menabo/ClonaRecord/{idTracciato}")]
        //public async Task<IActionResult> ClonaRecord(string req, int idTracciato)
        //{
        //    BoolResult res = new BoolResult();
        //    try
        //    {
        //        Dictionary<string, object>? elements = JsonConvert.DeserializeObject<Dictionary<string, object>>(req);
        //        var obj = elements!["obj"].ToString();
        //        var deserializedObj = JsonConvert.DeserializeObject<List<Dictionary<string, object>>>(obj!);
        //        List<PromoTracciatiRecord> recordToAdd = new List<PromoTracciatiRecord>();
        //        List<PromoTracciatiRecord> recordToRemove = new List<PromoTracciatiRecord>();
        //        string errorCodiciImpaginati = "";
        //        string errorCodiciFacentiParteDiUnAltroGruppo = "";
        //        foreach (var item in deserializedObj!)
        //        {


        //            var recordPresente = this.ctx2.PromoTracciatiRecords.Where(f => f.IdTracciato == idTracciato && f.Codice == item["Referenza.Codice"].ToString()).FirstOrDefault();

        //            if (recordPresente != null && recordPresente.CodiceGruppo == recordPresente.Codice)
        //            {
        //                if (recordPresente.Codice != recordPresente.CodiceGruppo)
        //                {
        //                    if (errorCodiciFacentiParteDiUnAltroGruppo == "")
        //                    {
        //                        errorCodiciFacentiParteDiUnAltroGruppo = "Impossibile procedere, i seguenti codici fanno già parte di un altro gruppo, procedere prima con lo sgruppamento: ";
        //                    }
        //                    errorCodiciFacentiParteDiUnAltroGruppo += "(Codice: " + recordPresente.Codice + " Gruppo: " + recordPresente.CodiceGruppo + ") ";
        //                    continue;
        //                }
        //                if (this.ctx2.MenaboRefs.Where(f => f.IdRecord == recordPresente.Id).Count() > 0)
        //                {
        //                    if (errorCodiciImpaginati == "")
        //                    {
        //                        errorCodiciImpaginati = "Impossibile procedere, rimuovere prima i seguenti codici dall'impaginato: ";
        //                    }
        //                    errorCodiciImpaginati += recordPresente.Codice + " ";
        //                    continue;
        //                }

        //                recordToRemove.Add(recordPresente);
        //            }

        //            if (errorCodiciImpaginati != "" || errorCodiciFacentiParteDiUnAltroGruppo != "")
        //            {
        //                continue;
        //            }


        //            var record = this.ctx2.PromoTracciatiRecords.Where(f => f.IdTracciato == int.Parse(item["idTracciato"].ToString()!) && f.Codice == item["Referenza.Codice"].ToString()).FirstOrDefault();
        //            if (record == null)
        //            {
        //                throw new Exception("Record " + item["Referenza.Codice"].ToString() + " non trovato nel tracciato numero " + item["idTracciato"].ToString());
        //            }
        ...
        //            {
        //                ////Console.WriteLine("Chiave: {0}, Valore: {1}", kvp.Key, kvp.Value);
        //                if (item.ContainsKey(kvp.Key))
        //                {
        //                    if (dato[kvp.Key] is bool)
        //                    {
        //                        if (bool.TryParse(item[kvp.Key].ToString(), out bool boolResult))
        //                            dato[kvp.Key] = boolResult;
        //                        else
        //                            dato[kvp.Key] = default(bool);
        //                    }
        //                    else if (dato[kvp.Key] is int)
        //                    {
        //                        if (int.TryParse(item[kvp.Key].ToString(), NumberStyles.Any, numberFormatInfo, out int intResult))
        //                            dato[kvp.Key] = intResult;
        //                        else
        //                            dato[kvp.Key] = default(int);
        //                    }
        //                    else if (dato[kvp.Key] is float)
        //                    {
        //                        if (float.TryParse(item[kvp.Key].ToString(), NumberStyles.Any, numberFormatInfo, out float floatResult))
        //                            dato[kvp.Key] = floatResult;
        //                        else
        //                            dato[kvp.Key] = default(float);
        //                    }
        //                    else if (dato[kvp.Key] is decimal)
        //                    {
        //                        if (decimal.TryParse(item[kvp.Key].ToString(), NumberStyles.Any, numberFormatInfo, out decimal decimalResult))
        //                            dato[kvp.Key] = decimalResult;
        //                        else
        //                            dato[kvp.Key] = default(decimal);
        //                    }
        //                    else if (dato[kvp.Key] is double)
        //                    {
        //                        if (double.TryParse(item[kvp.Key].ToString(), NumberStyles.Any, numberFormatInfo, out double doubleResult))
        //                            dato[kvp.Key] = doubleResult;
        //                        else
        //                            dato[kvp.Key] = default(double);
        //                    }
        //                    else
        //                    {
        //                        dato[kvp.Key] = item[kvp.Key]; // Assegna il valore così com'è se non è uno dei tipi specificati
        //                    }
        //                }
        //            }
        ...
        //            newRecord.IndiceLettura = record.IndiceLettura;
        //            newRecord.Label = record.Label;
        //            newRecord.DaEsportare = record.DaEsportare;
        //            newRecord.Scatto = record.Scatto;
        //            newRecord.Codice = record.Codice;
        //            newRecord.CodiceGruppo = item["Scatto.CodiceGruppo"].ToString();
        //            newRecord.DataRegistrazione = DateTime.Now;
        //            newRecord.IdTracciato = idTracciato;
        //            newRecord.IndiceEsportazione = record.IndiceEsportazione;
        //            newRecord.ModalitaInserimento = (byte)ModalitaInserimento.Manuale;
        //            //newRecord.SelezioneMenabo = record.SelezioneMenabo;
        ...
        //            newRecord.Dato = JsonConvert.SerializeObject(dato);
        //            recordToAdd.Add(newRecord);
        //        }

        //        if (errorCodiciFacentiParteDiUnAltroGruppo != "")
        //        {
        //            res.Esito = false;
        //            res.error = errorCodiciFacentiParteDiUnAltroGruppo;
        //            return Ok(res);
        //        }
        ...
        //        {
        //            res.Esito = false;
        //            res.error = errorCodiciImpaginati;
        //            return Ok(res);
        //        }

        //        this.ctx2.PromoTracciatiRecords.AddRange(recordToAdd);
        //        this.ctx2.PromoTracciatiRecords.RemoveRange(recordToRemove);
        //        this.ctx2.SaveChanges();
        //        res.Esito = true;
        //    }
        //    catch (Exception ex)
