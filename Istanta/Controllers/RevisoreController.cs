...
namespace Istanta.Controllers
{
	...
	public class RevisoreController : Controller
    {
        ...
        //        //q_records.AddRange(dati.Where(d => d[GLOBAL_VARIABLES_FICO.keyCodiceGruppo].ToString()== "4696706,4696730").Select(s =>
        //        {
        ...
        //        await Utility.Selezionatore.selezioneAutomaticaRefInMenabo(q_records, this.ctx2, this._fico_conf.Value.nomeCliente, this.path_external_lib, true);


        //        var loro = q_records.Where(f => f.Dato["Scatto.CodiceGruppo"].ToString()!.Contains("4696730")).ToList();
        ...
        //        List<Articoli> arts = await this.ctx.Articolis.Include(i => i.ArticoliDescrizionis).Include(i=> i.ArticoliFotos).Where(r => q_records_codici.Contains(r.Codice)).ToListAsync();
        ...
        //            {
        //                var listaRecordsComuni = q_records.Where(t => t.Dato[key_codice_ref].ToString() == cod).ToList();
        //                var listaACComuni = listaRecordsComuni.Select(s => new ACcomuni
        //                {
        //                    NomeAC = s.Canale + s.Area,
        //                    canale = s.Canale,
        //                    area = s.Area,
        //                    InVol = true
        //                }
        ...
        //                listaRecordsComuni.ForEach(f => f.Dato[GLOBAL_VARIABLES.keyACComuni] = listaACComuni);
        //            }
        //        }

        //        var gruppi = q_records.GroupBy(r => new
        //        {
        //            Gruppo = (string)r.Dato[key_codice_gruppo],
        //            r.Canale,
        //            r.Area
        //        });
        ...
        //        {
        //            if (g.FirstOrDefault(f => !f.Dato.ContainsKey(GLOBAL_VARIABLES.keyXMLSelezione)) != null)
        //            {
        //                continue;
        //            }
        //            // Primario: prendi il primo (se esiste), altrimenti stringa vuota
        //            var primario = g.FirstOrDefault(r => r.Dato.ContainsKey(GLOBAL_VARIABLES.keyXMLSelezione) && (byte)r.Dato[GLOBAL_VARIABLES.keyXMLSelezione] == 1);
        //            var primarioRef = primario != null ? (string)primario.Dato[key_codice_ref] : string.Empty;

        //            // Secondari: tutti con StatoSelezione == 2
        //            var secondariJoin = string.Join(",",
        //                g.Where(r => r.Dato.ContainsKey(GLOBAL_VARIABLES.keyXMLSelezione) && (byte)r.Dato[GLOBAL_VARIABLES.keyXMLSelezione] == 2)
        //                 .Select(r => (string)r.Dato[key_codice_ref])
        //            );

        //            foreach (var r in g)
        //            {
        //                r.Dato["PrimarioGruppo"] = primarioRef;      // "" se assente
        //                r.Dato["SecondariGruppo"] = secondariJoin;    // "" se nessun secondario
        //            }
        //        }

        //        var tracciatoSingoli = new List<q_records_per_getListaRevisione>();


        //        var groups = q_records
        //            .GroupBy(g => (
        //                Canon(GetDatoValueOrNull(g.Dato, key_codice_ref)!),
        //                Canon(GetDatoValueOrNull(g.Dato, key_codice_gruppo)!)
        //            ));
        ...
        //                var valueGroups = grp
        //                    .GroupBy(r => Canon(GetDatoValueOrNull(r.Dato, k)!))
        //                    .ToList();
        ...
        //                        var raw = GetDatoValueOrNull(vg.First().Dato, k);

        //                        return new Dictionary<string, object>
        //                        {
        //                            ["value"] = raw!,                    // un esempio “raw” del valore
        //                            ["origins"] = vg.Select(r => r.Canale+ r.Area)
        //                                             .Distinct()
        //                                             .ToList()
        //                        };
        //                    }).ToList();
        ...
        //            {
        //                r.Dato["origine"] = r.Canale+ r.Area;
        //                r.Dato["metaKeyInMismatch"] = mismatchDetails
        ...
        //        objParams.Add("tracciatoSingoli", tracciatoSingoli);
        //        objParams.Add("listaOrigine", q_records);


        //        tracciatoSingoli = (icItem.execLibFunction($"AgenziaLib.{this._fico_conf.Value.nomeCliente}.specificaInOutVol", objParams) as List<q_records_per_getListaRevisione>);
        ...
        //            var items = tracciatoSingoli!.Where(t => t.Dato[key_codice_gruppo].ToString() == cod_gruppo);
        ...
        //                Articoli artItem = arts.Where(a => a.Codice == item2.Dato[key_codice_ref].ToString()).FirstOrDefault()!;
        //                if (artItem != null)
        //                {
        ...
        //                //var itemSingolo = tracciatoSingoli.Where(t => t.Dato[key_codice_gruppo]! == cod_gruppo && t.Dato.ContainsKey(scope_descrizioni!) && (t.Dato[scope_descrizioni!]! as JObject)!.ToObject<DescrittivaArticolo>()!.gruppo != null).FirstOrDefault()!;
        //                var itemSingolo = tracciatoSingoli!.Where(t => t.Dato[key_codice_gruppo].ToString() == cod_gruppo).FirstOrDefault()!;
        //                itemG.recordInTracciato = itemSingolo?.Dato!;
        ...
        //                var naz = archDecrGruppo.Where(ar => ar.CodiceGruppo == cod_gruppo && ar.Area == null && ar.Canale == null && ar.Custom != null).FirstOrDefault();
        //                if (naz == null)
        //                {
        //                    naz = archDecrGruppo.Where(ar => ar.CodiceGruppo == cod_gruppo && ar.Area == null && ar.Canale == null && ar.Custom == null).FirstOrDefault();
        //                }
        //                itemG.recordRevisionato = naz!;
        //                var descrReg = archDecrGruppo.Where(f => f.Canale != null || f.Area != null).OrderByDescending(o => o.DataUltimaRicezione).ToList();
        ...
        //        //await Utility.Selezionatore.selezioneAutomaticaRefInMenabo(result.Data, this.ctx2, this._fico_conf.Value.nomeCliente, this.path_external_lib, true);
...
out var mismatch);
    ...
    .ToDictionary(
        ...
        //    // Sottogruppi richiesti (codici gruppo, es: "cod1,cod2,cod3")
        //    var sottogruppi = codiciList.stringhe?
        //        .Where(s => !string.IsNullOrWhiteSpace(s))
        //        .Distinct(StringComparer.OrdinalIgnoreCase)
        //        .ToList() ?? new List<string>();

        //    // Expected codici singoli per ciascun sottogruppo
        //    var expectedCodesBySg = sottogruppi.ToDictionary(
        //        sg => sg,
        //        sg => sg.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)
        //                .Where(x => !string.IsNullOrWhiteSpace(x))
        //                .ToHashSet(StringComparer.OrdinalIgnoreCase),
        //        StringComparer.OrdinalIgnoreCase
        //    );

        //    // Lista di codici singoli totale (per filtrare la query sui records)
        //    var codiciSingoli = expectedCodesBySg.Values
        //        .SelectMany(set => set)
        //        .Distinct(StringComparer.OrdinalIgnoreCase)
        //        .ToList();

        //    // Tracciati associati alla promo
        //    var promoTracciatiIds = await this.ctx2.PromoTracciatis
        //        .Where(t => t.IdPromo == idPromo)
        //        .Select(t => t.Id)
        //        .ToListAsync();

        //    // Records filtrati: solo codici singoli di interesse e solo tracciati della promo
        //    var promoTracciatiRecords = await this.ctx2.PromoTracciatiRecords
        //        .Where(r => promoTracciatiIds.Contains(r.IdTracciato) && codiciSingoli.Contains(r.Codice))
        //        .ToListAsync();

        //    // Descrizioni: per codice gruppo (come da tua conferma)
        //    var items = await this.ctx.ArticoliDescrizionis
        //        .Where(a => codiciList.stringhe!.Contains(a.CodiceGruppo!))
        //        .ToListAsync();

        //    // Calcolo firma per ciascun sottogruppo: prendo il PRIMO tracciato che matcha completo
        //    var firmaBySg = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

        //    foreach (var tracciatoGroup in promoTracciatiRecords.GroupBy(r => r.IdTracciato))
        //    {
        //        if (firmaBySg.Count == sottogruppi.Count)
        //            break;

        //        // Per questo tracciato: sg -> (codiceSingolo -> dict)
        //        var dictsBySg = new Dictionary<string, Dictionary<string, Dictionary<string, object>>>(StringComparer.OrdinalIgnoreCase);

        //        foreach (var r in tracciatoGroup)
        //        {
        //            var dict = Utility.Main.getJsonObject(r.Dato);
        //            if (dict == null) continue;

        //            if (!dict.TryGetValue(kSottogruppo, out var v) || v == null) continue;

        //            var sgValue = v.ToString();
        //            if (string.IsNullOrWhiteSpace(sgValue)) continue;

        //            // ci interessa solo se è uno dei sottogruppi richiesti e non già risolto
        //            if (!expectedCodesBySg.ContainsKey(sgValue)) continue;
        //            if (firmaBySg.ContainsKey(sgValue)) continue;

        //            if (!dictsBySg.TryGetValue(sgValue, out var byCode))
        //            {
        //                byCode = new Dictionary<string, Dictionary<string, object>>(StringComparer.OrdinalIgnoreCase);
        //                dictsBySg[sgValue] = byCode;
        //            }

        //            // tieni il primo dict per quel codice singolo (se duplicati, ignora i successivi)
        //            if (!byCode.ContainsKey(r.Codice))
        //                byCode[r.Codice] = dict;
        //        }

        //        // Provo a chiudere i sottogruppi per questo tracciato
        //        foreach (var kv in dictsBySg)
        //        {
        //            var sg = kv.Key;
        //            var byCode = kv.Value;

        //            var expectedSet = expectedCodesBySg[sg];

        //            // match esatto: devono esserci tutti e solo quelli
        //            if (byCode.Count != expectedSet.Count) continue;
        //            if (!expectedSet.All(code => byCode.ContainsKey(code))) continue;

        //            var membriGruppo = expectedSet
        //                .OrderBy(x => x, StringComparer.OrdinalIgnoreCase)
        //                .Select(code => byCode[code])
        //                .ToList();

        //            var hash = Utility.Main.getFirmaTracciatoGruppo(membriGruppo);
        //            firmaBySg[sg] = hash;
        //        }
        //    }

        //    // Costruisco output finale
        //    var output = sottogruppi
        //        .Select(sg => new DescrizioniSottogruppi
        //        {
        //            codiceSottogruppo = sg,
        //            descrizioni = items.Where(a => string.Equals(a.CodiceGruppo, sg, StringComparison.OrdinalIgnoreCase)).ToList(),
        //            firmaTracciatoSottogruppo = firmaBySg.TryGetValue(sg, out var firma) ? firma : ""
        //        })
        //        .ToList();
        ...
        //[HttpPut]
        //[Route("Revisore/getArticoliDescrizioni")]
        //public async Task<IActionResult> getArticoliDescrizioni(StringList codiciList)
        //{
        //    List<ArticoliDescrizioni> items = await this.ctx.ArticoliDescrizionis.Include(f=>f.IdArticoloNavigation).Where(f => codiciList.stringhe!.Contains(f.CodiceGruppo!) || (f.IdArticoloNavigation != null && codiciList.stringhe.Contains(f.IdArticoloNavigation.Codice))).ToListAsync();
        //    return Ok(items);
        //}
    ...
    bool isSottogruppo)
        ...
        //                                                {
        //                                                    //string codArtNewFoto = foto.ContainsKey(GLOBAL_VARIABLES.keySyncFotoInddCodice)?foto[GLOBAL_VARIABLES.keySyncFotoInddCodice].ToString():"";
        ...
        //                                                    string normilezed = nome_foto.Normalize(System.Text.NormalizationForm.FormC);
        //                                                    if (normilezed!=nome_foto)
        //                                                    {
								//								nome_foto = normilezed;
								//							}
        ...
        //[HttpPut]
        //[Route("Revisore/SincronizzaModificheIndd/{stato}/{areaRichiesta}/{canaleRichiesto}/{idOperazione}")]
        //public async Task<IActionResult> SincronizzaModificheIndd(string SyncIndd, int stato, bool areaRichiesta = false, bool canaleRichiesto = false, int idOperazione = 0)
        //{
        //    BoolResult result = new BoolResult();
        ...
        //    string key_codice_ref = Enum.GetName(AddestramentoRuoli.Referenza) + "." + GLOBAL_VARIABLES.keyRefCodice;
        //    string key_codice_gruppo = Enum.GetName(AddestramentoRuoli.Scatto) + "." + GLOBAL_VARIABLES.keyScattoCodiceGruppo;
        //    string key_descr_indd = Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescrIndd;
        //    string keySyncIndd = GLOBAL_VARIABLES.keySyncIndd;
        //    string key_descr1 = Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr1;
        //    string key_descr2 = Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr2;
        //    string key_descr3 = Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr3;
        //    string key_descr4 = Enum.GetName(AddestramentoRuoli.Descrizioni) + "." + GLOBAL_VARIABLES.keyDescr4;
        //    string key_tracciato_firma = Enum.GetName(AddestramentoRuoli.Tracciato) + "." + GLOBAL_VARIABLES.keyTracciatoFirma;
        //    string key_idRec = GLOBAL_VARIABLES.keyRefIdRec;
        //    //string erroriRiscontrati = "";
        //    try
        //    {

        //        Dictionary<string, object> Indd = new Dictionary<string, object>();
        //        Indd = JsonConvert.DeserializeObject<Dictionary<string, object>>(SyncIndd);
        //        RevisioniActions actions = new RevisioniActions();
        //        RevisioneAction act = new RevisioneAction();
        //        var primario = this.ctx2.PromoTracciatiRecords.Include(t=>t.IdTracciatoNavigation).Where(f => f.Id == long.Parse(Indd[key_idRec].ToString())).FirstOrDefault();
        //        if(primario == null)
        //        {
        //            throw new Exception("Record non trovato in promo tracciati record");
        //        }
        //        int id_tracciato = primario.IdTracciato;

        //        if (stato==1 || stato == 3)
        //        {
        //            if (Indd["codice_gruppo"].ToString().Contains(","))
        //            {
        //                records.AddRange(this.ctx2.PromoTracciatiRecords.Where(f => f.CodiceGruppo == Indd["codice_gruppo"].ToString() && f.IdTracciato == id_tracciato).ToList());
        //                act.CodiceGruppo = Indd["codice_gruppo"].ToString();
        //                act.DescrizioneIndd = Indd[key_descr_indd].ToString();
        //                act.Descrizione1 = Indd[key_descr1].ToString();
        //                act.Descrizione2 = Indd[key_descr2].ToString();
        //                act.Descrizione3 = Indd[key_descr3].ToString();
        //                act.Descrizione4 = Indd[key_descr4].ToString();
        //                act.Area = Indd[key_descr4].ToString();
        //                act.Canale = primario.IdTracciatoNavigation.Canale;
        //                act.Area = primario.IdTracciatoNavigation.Area;
        //                act.areaRichiesta = areaRichiesta;
        //                act.canaleRichiesto = canaleRichiesto;

        //                using (var md5 = System.Security.Cryptography.MD5.Create())
        //                {
        //                    string firmaGruppo = "";
        //                    records = records.OrderBy(f => f.Codice).ToList();
        //                    foreach (var rec in records)
        //                    {
        //                        var dato = IstantaJson.getJsonObject(rec.Dato);
        //                        if (dato.ContainsKey(key_tracciato_firma))
        //                        {
        //                            firmaGruppo += dato[key_tracciato_firma].ToString();
        //                        }
        //                        else
        //                        {
        //                            throw new Exception(rec.Codice + " non ha nel Dato la chiave " + key_tracciato_firma);
        //                        }
        //                    }
        ...
        //                    act.FirmaTracciato = firmaGruppo;
        //                }
        //                //md5
        //            }
        //            else
        ...
        //                records.Add(primario);
        //                var art = this.ctx.Articolis.Where(f => f.Codice == Indd["codice_gruppo"].ToString()).FirstOrDefault();
        //                if (art != null)
        //                {
        //                    act.IdRef = art.Id;
        //                    act.DescrizioneIndd = Indd[key_descr_indd].ToString();
        ...
        //                    act.Descrizione3 = Indd[key_descr3].ToString();
        //                    act.Descrizione4 = Indd[key_descr4].ToString();
        //                    var dato = IstantaJson.getJsonObject(primario.Dato);
        //                    act.Canale = primario.IdTracciatoNavigation.Canale;
        //                    act.Area = primario.IdTracciatoNavigation.Area;
        //                    act.areaRichiesta = areaRichiesta;
        //                    act.canaleRichiesto = canaleRichiesto;
        //                    act.FirmaTracciato = (dato.ContainsKey(key_tracciato_firma) ? dato[key_tracciato_firma].ToString() : "");
        //                }
        //                else
        //                {
        //                    throw new Exception("Articolo non trovato nel database Articoli di istanta");
        //                }
        //            }
        //            actions.coda.Add(act);
        //            IActionResult resSalvataggio = await salva(actions, id_tracciato, idOperazione, senderOperazione.indd);
        //            if (resSalvataggio is OkObjectResult)
        //            {
        //                var okResult = resSalvataggio as OkObjectResult;
        //                if (okResult.Value is BoolResult)
        //                {
        //                    var esito = (okResult.Value as BoolResult);
        //                    if (!esito.Esito)
        //                    {
        //                        throw new Exception("Fallimento della revisione dell'elemento " + Indd["codice_gruppo"].ToString() + " errore: " + esito.error);
        //                    }
        //                }
        //            }
        ...
        //        if(stato == 2 || stato == 3)
        //        {
        //            Dictionary<string, object> campiOfferteFormattatiPerSyncIndd = new Dictionary<string, object>();
        //            foreach (var item in Indd)
        //            {
        //                var innerDict = (item.Value as Dictionary<string, string>);
        //                if (item.Key == "meccanica")
        //                {
        //                    campiOfferteFormattatiPerSyncIndd.Add(item.Key, item.Value);
        //                }
        //                else if (innerDict.ContainsKey("valore") && innerDict.ContainsKey("campoUniversale"))
        //                {
        //                    campiOfferteFormattatiPerSyncIndd.Add(innerDict["campoUniversale"], innerDict["valore"]);
        //                }
        //            }
        ...
        //                dato[keySyncIndd] = campiOfferteFormattatiPerSyncIndd;
        //                record.Dato = JsonConvert.SerializeObject(dato);
        //            }
        //            this.ctx2.SaveChanges();

        //            var register = new Register(_config.GetConnectionString("IstandaConnectionDb"));
        //            if (idOperazione == 0)
        //            {
        //                //l'operazione è stata autorizzata per cui procediamo a creare l'operazione sul DB
        //                var nuovaOperazione = new RegistroOperazioni();
        //                nuovaOperazione.Area = areaRichiesta ? primario.IdTracciatoNavigation.Area : "";
        //                nuovaOperazione.Canale = canaleRichiesto ? primario.IdTracciatoNavigation.Canale : "";
        //                nuovaOperazione.Stato = (byte)statoOperazioni.risolta;
        //                nuovaOperazione.Autore = int.Parse(session);
        //                var data = DateTime.Now;
        ...
        //                nuovaOperazione.TipoOperazione = (byte)tipoOperazione.revisioneCampiOfferta;
        //                nuovaOperazione.IdTracciato = id_tracciato;
        //                nuovaOperazione.CodiceAssociato = Indd["codice_gruppo"].ToString();
        //                nuovaOperazione.FormData = SyncIndd;
        //                nuovaOperazione.Url = "Revisore/SincronizzaModificheIndd";
        //                register.addOperazione(nuovaOperazione, true, session, DateTime.Now);
        //            }
        //            else
        //            {
        ...
        //private BoolResult registraOperazioneDiRevisioneCampiOffertaSuLavorazioni(PromoLavorazioniRecord plr, string codice, RevisioneCampiOffertaFromIndd formData)
        //{
        //    BoolResult result = new BoolResult();
        ...
        //        Register regContext=new Register(this.connString, this._dbContextFactory);

        //        RegistroOperazioni regOp = new RegistroOperazioni();

        //        regOp.CodiceAssociato = codice;
        //        regOp.IdTracciato= plr.IdPromoTracciatiRecordNavigation.IdTracciato;
        //        regOp.idPromoLavorazioniRecord=plr.Id;
        //        regOp.idPromoLavorazione = plr.IdLavorazione;
        //        regOp.TipoOperazione = (byte)tipoOperazione.revisioneCampiOfferta;
        //        regOp.FormData = JsonConvert.SerializeObject(formData);
        //        regOp.CodiceAssociato = codice;


        //        regContext.addOperazione(regOp, true, session, DateTime.Now);
        ...
        //private BoolResult registraOperazioneDiRevisioneDescrizioneSuLavorazioni(PromoLavorazioniRecord plr, string codice, RevisioneDescrizione formData)
        //{
        //    BoolResult result = new BoolResult();
        ...
        //        regOp.idPromoLavorazione = plr.IdLavorazione;
        //        regOp.idPromoLavorazioniRecord = plr.Id;
        //        regOp.TipoOperazione = (byte)tipoOperazione.revisione;
        //        regOp.FormData = JsonConvert.SerializeObject(formData);
        //        regOp.CodiceAssociato = codice;
