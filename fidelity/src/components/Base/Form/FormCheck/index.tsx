...
FormCheck.Label = (props: LabelProps & {formLabelSize? : "sm" | "lg"}) => {
  const sizeClass = props.formLabelSize === "sm" ? "text-sm" : props.formLabelSize === "lg" ? "text-lg" : "";
  return (
    <label
      {...props}
      className={twMerge(["cursor-pointer ml-2", props.className, sizeClass])}
    >
      {props.children}
    </label>
  );
