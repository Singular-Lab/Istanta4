...
interface StartRevisionDialogProps {
    mutationAvvioRevisione: ReturnType<typeof useMutation>;
    dialogOpen: boolean;
    setDialogOpen: (open: boolean) => void;
    lavorazione: LavorazioneType;
}

const StartRevisionDialog = ({ mutationAvvioRevisione, dialogOpen, setDialogOpen, lavorazione }: StartRevisionDialogProps) => (
    <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)}>
        <Dialog.Panel>
            <Dialog.Title>
                <div className="flex items-center">
                    <Lucide icon="FileCheck" className="w-5 h-5 text-info mr-2" />
                    <h2 className="text-lg font-semibold text-slate-800 text-center">Sottoponi a revisione</h2>
                </div>
            </Dialog.Title>
            <Dialog.Description className="text-center p-2">
                <div className="bg-info/5 p-3 rounded-lg border border-info/10 mb-3">
                    <p className="text-sm text-slate-700">
                        Stai per avviare la revisione per:
                    </p>
                ...
                <p className="text-xs text-slate-500">
                    La revisione permetterà di verificare e approvare i file caricati.
                </p>
            </Dialog.Description>
            <Dialog.Footer className="flex justify-end gap-4">
                ...
                <Button
                    ...
                    onClick={() => {
                        setDialogOpen(false);
                        mutationAvvioRevisione.mutate(lavorazione.guidId);
                    }}
                >
...
export default StartRevisionDialog; 