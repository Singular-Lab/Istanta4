    ...
    <div className="box box--stacked p-5 mb-5">
        {(title || right) && (
            <div className="flex items-center justify-between mb-3 gap-3">
                {title && <h3 className="text-[0.94rem] font-medium text-slate-800">{title}</h3>}
                {right}
            </div>
        )}
...
const NuovaVersioneTemplatesWhatsappSuperAdmin: React.FC = () => {
    const [form, setForm] = useState<FormState>(initial);
    const [errors, setErrors] = useState<Record<string, string>>({});
    const [isDialogApprovazioneOpen, setIsDialogApprovazioneOpen] = useState(false);
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [submitError, setSubmitError] = useState("");
    const [submitSuccess, setSubmitSuccess] = useState(false);

    const [isSavedInLocale, setIsSavedInLocale] = useState(false);
    ...
    const { template } = useLoaderData() as { template?: GDOWhatsappTemplateAttributes };
    const { revalidate } = useRevalidator();

    const [isJsonModified, setIsJsonModified] = useState(false);
    const [isInApproval, setIsInApproval] = useState(false);

    // mappo stato di approvazione dal template originale
    useEffect(() => {
        if (!template) {
            setIsInApproval(false);
            return;
        }
        setIsInApproval(template.stato_meta_gdowhatsapptemplate === STATO_GDO_WHATSAPP_TEMPLATE.PENDING);
    }, [template]);
    ...
    useEffect(() => {
        if (!template) return;
        try {
            const t = template.json_meta_gdowhatsapptemplate as json_meta_gdowhatsapptemplate | undefined;
            if (!t) return;

            setForm((prev) => {
                const header = t.components.find((c) => c.type === "HEADER") as
                    | Extract<WhatsAppTemplateComponent, { type: "HEADER" }>
                    | undefined;
                const body = t.components.find((c) => c.type === "BODY") as
                    | Extract<WhatsAppTemplateComponent, { type: "BODY" }>
                    | undefined;
                const footer = t.components.find((c) => c.type === "FOOTER") as
                    | Extract<WhatsAppTemplateComponent, { type: "FOOTER" }>
                    | undefined;
                const buttons = t.components.find((c) => c.type === "BUTTONS") as
                    | Extract<WhatsAppTemplateComponent, { type: "BUTTONS" }>
                    | undefined;

                const headerFormat: HeaderFormat = header?.format ? (header.format as HeaderFormat) : "NONE";
                const headerText = header?.text || "";
                const headerSampleMediaUrl = header?.example?.header_handle?.[0];
                ...
                let buttonKind: ButtonKind = "NONE";
                let quickReplies: QuickReply[] = [];
                let ctas: Cta[] = [];

                if (buttons && Array.isArray(buttons.buttons) && buttons.buttons.length) {
                    const allQR = buttons.buttons.every((b) => b.type === "QUICK_REPLY");
                    const allCTA = buttons.buttons.every(
                        (b) => b.type === "URL" || b.type === "PHONE_NUMBER"
                    );
                    if (allQR) {
                        buttonKind = "QUICK_REPLY";
                        quickReplies = (buttons.buttons as any[]).map((b) => ({
                            type: "QUICK_REPLY",
                            text: b.text,
                        }));
                    } else if (allCTA) {
                        buttonKind = "CTA";
                        ctas = (buttons.buttons as any[]).map((b) =>
                            b.type === "URL"
                ...
                return {
                    ...prev,
                    name: template.nome_template_gdowhatsapptemplate || t.name || prev.name,
                    language:
            ...
            console.warn("Impossibile mappare il template esistente:", err);
        }
    }, [template]);
    ...
    function buildPayload(): json_meta_gdowhatsapptemplate {
        ...
        if (form.headerFormat === "TEXT") {
            ...
            components.push({
                ...
                format,
                example: {
                    header_handle: form.headerSampleMediaUrl ? [form.headerSampleMediaUrl] : [],
                },
            });
        }

        const body: Extract<WhatsAppTemplateComponent, { type: "BODY" }> = {
            type: "BODY",
            text: form.bodyText,
        };
    ...
    // Controllo se il JSON è stato modificato rispetto all'originale
    useEffect(() => {
        if (!template) return;
        try {
            const originalJson = JSON.stringify(template.json_meta_gdowhatsapptemplate ?? {});
            const currentJson = JSON.stringify(buildPayload());
            setIsJsonModified(originalJson !== currentJson);
        } catch {
    ...
    const validation = useMemo(() => {
        ...
        if (form.headerFormat === "TEXT") {
            ...
            if (form.headerText.length > MAX_HEADER_TEXT)
                e.headerText = `Max ${MAX_HEADER_TEXT} caratteri.`;
        } else if (["IMAGE", "DOCUMENT", "VIDEO"].includes(form.headerFormat)) {
            if (!form.headerSampleMediaUrl?.trim()) {
                e.headerSampleMediaUrl =
                    "Per i media header serve un esempio (URL/handle) per l'approvazione.";
            }
        }
        ...
        if (needBodyExamples) {
            if (
                form.bodyExamples.length !== placeholders.length ||
        ...
        if (form.footerText.length > MAX_FOOTER_TEXT) {
            e.footerText = `Footer max ${MAX_FOOTER_TEXT} caratteri.`;
        }
        ...
        if (form.buttonKind === "QUICK_REPLY") {
            ...
            if (
                form.quickReplies.some(
                    (q) => !q.text.trim() || q.text.length > 25
                )
            ) {
                e.quickReplies =
                    "Testo quick reply obbligatorio (consigliato ≤25 caratteri).";
            }
        }
            ...
            for (const [i, c] of form.ctas.entries()) {
                ...
                if (c.type === "URL") {
                    if (!isDynamicUrlValid(c.url)) {
                        e[`cta_${i}_url`] =
                            "URL non valido. Puoi usare segnaposto {{n}} nel path/query.";
                    }
                } else if (c.type === "PHONE_NUMBER") {
                    if (!c.phone_number?.trim()) {
                        e[`cta_${i}_phone`] =
                            "Numero di telefono obbligatorio (es. +39055...).";
                    }
                }
            }
        }
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        // il submit vero è gestito dai bottoni (salva / invia)
    }

    async function salvaInLocale(payload: json_meta_gdowhatsapptemplate) {
        try {
            setIsLoadingSave(true);
            const res = await ServerCall.put<{
                success: boolean;
                newTemplateId: string;
                error?: string;
            }>("/whatsapp/nuova_versione_locale_template_whatsapp", {
            ...
            // Naviga alla pagina con il nuovo id e revalida
            window.history.replaceState(
                {},
                "",
                `/whatsapp/gestione-whatsapp-superadmin/templates/nuova-versione?id=${res.newTemplateId}`
            );
            setIsLoadingSave(false);
            setIsSavedInLocale(true);
            revalidate();

        } catch (error) {
            setIsLoadingSave(false);
            setIsSavedInLocale(false);
        }
    }

    async function sincronizzaTemplateConMeta() {
        try {
            setIsLoadingSave(true);
            await ServerCall.put("/whatsapp/send_modified_template_to_meta", {
                id_template: template?.id_gdowhatsapptemplate,
            });
            setIsLoadingSave(false);
            setIsSavedInLocale(true);
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex-1">
                    <PageHeader
                        title="Nuova versione Template WhatsApp"
                        description="Configura una nuova versione del template, pronta per l’approvazione Meta."
                    />
                </div>

                <div className="flex flex-wrap items-center gap-2 lg:justify-end">
                    {isSavedInLocale && (
                        <Badge size="sm" variant="success" border>
                            Salvato in locale
                        </Badge>
                    )}
                    {isInApproval && (
                        <Badge size="sm" variant="warning" border>
                            In revisione Meta
                        </Badge>
                    )}
                </div>
            ...
            {/* DIALOG INVIO META */}
            <Dialog open={isDialogApprovazioneOpen} onClose={() => setIsDialogApprovazioneOpen(false)} size="md">
                <Dialog.Panel>
                    <Dialog.Title>
                        <h3 className="text-lg font-medium">Invia per approvazione</h3>
                    </Dialog.Title>
                    <Dialog.Description>
                        Sei sicuro di voler inviare questo template a Meta per l’approvazione?
                        Una volta inviato, non potrai modificarlo fino alla fine della revisione.
                        {errors && Object.keys(errors).length > 0 && (
                            <div className="text-danger text-sm mt-2">
                                Correggi gli errori nel form prima di inviare.
                            </div>
                        )}
                        {isSubmitting && (
                            <Alert className="mt-2" variant="outline-pending">
                                Invio in corso...
                            </Alert>
                        )}
                        {submitError && (
                    ...
                    <Dialog.Footer className="flex justify-end gap-2">
                        <Button
                            variant="secondary"
                            onClick={() => setIsDialogApprovazioneOpen(false)}
                        ...
                        <Button
                            ...
                            onClick={async () => {
                                ...
                                try {
                                    await sincronizzaTemplateConMeta();
                                    setSubmitSuccess(true);
                                    setTimeout(() => {
                                        setIsDialogApprovazioneOpen(false);
                                        setSubmitSuccess(false);
                                    }, 1500);
                                } catch (err: any) {
            ...
            {/* ALERT STATO MODIFICA */}
            {template?.stato_meta_gdowhatsapptemplate ===
                STATO_GDO_WHATSAPP_TEMPLATE.CREATED_BUT_NOT_SYNCED && (
                    <Alert className="flex flex-row items-center" variant="soft-warning">
                        <span className="mr-2 flex items-center">
                            <Lucide icon="TriangleAlert" className="w-8 h-8" />
                        </span>
                        Il template è stato modificato ma non è ancora sincronizzato con Meta.
                        Fino a che il template non viene inviato, al template non gli verrà aumentata la versione.
                    </Alert>
                )}

            {/* LAYOUT A DUE COLONNE: FORM + PREVIEW */}
            <form onSubmit={handleSubmit}>
                <div className="grid gap-5 lg:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
                    ...
                    <div className="space-y-4">
                        <Card title="Informazioni generali">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <FormInput
                                        placeholder="es. promo_fidelity_settembre"
                                        value={form.name}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <FormSelect
                                        ...
                                        onChange={(e) =>
                                            update("headerFormat", e.target.value as HeaderFormat)
                                        }
                                    >
                                ...
                                {form.headerFormat === "TEXT" && (
                                    <div className="md:col-span-2">
                                        ...
                                        <FormInput
                                            placeholder="Es. Istanta 2 GDO Suite"
                                            value={form.headerText}
                                            onChange={(e) =>
                                                update("headerText", e.target.value)
                                            }
                                        />
                                        {errors.headerText && (
                                            <ErrorMsg>{errors.headerText}</ErrorMsg>
                                        )}
                                    </div>
                                )}
                                ...
                                {["IMAGE", "DOCUMENT", "VIDEO"].includes(form.headerFormat) && (
                                    <div className="md:col-span-2">
                                        ...
                                        <FormInput
                                            placeholder="https://… (consigliato per approvazione)"
                                            value={form.headerSampleMediaUrl || ""}
                                            onChange={(e) =>
                                                update("headerSampleMediaUrl", e.target.value)
                                            }
                                        />
                                        <HelperLabel>
                                            Per i media header, Meta richiede un esempio per la
                                            review.
                                        </HelperLabel>
                                        {errors.headerSampleMediaUrl && (
                                            <ErrorMsg>{errors.headerSampleMediaUrl}</ErrorMsg>
                                        )}
                                    </div>
                                )}
                            </div>
                        ...
                        <Card
                            title="Body"
                            right={<Counter value={form.bodyText} max={MAX_BODY} />}
                        >
                            <FormTextarea
                                rows={6}
                                placeholder={`Es. Ciao {{1}}, la tua fidelity è attiva! Sconto del {{2}}% su volantini web.\nValido fino al {{3}}.`}
                                value={form.bodyText}
                                onChange={(e) => update("bodyText", e.target.value)}
                            />
                            <HelperLabel>
                                Usa segnaposto sequenziali: <code>{`{{1}}`}</code>,{" "}
                                <code>{`{{2}}`}</code>…; niente salti.
                            </HelperLabel>
                            ...
                            {needBodyExamples && (
                                <div className="mt-3">
                                    ...
                                    {errors.bodyExamples && (
                                        <ErrorMsg>{errors.bodyExamples}</ErrorMsg>
                                    )}
                                </div>
                            )}
                        </Card>
                        ...
                        <Card
                            title="Footer"
                            right={<Counter value={form.footerText} max={MAX_FOOTER_TEXT} />}
                        >
                            <FormInput
                                placeholder="Es. Servizio clienti 9-18"
                        ...
                        <Card title="Bottoni">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <FormSelect
                                        value={form.buttonKind}
                                        onChange={(e) =>
                                            update("buttonKind", e.target.value as ButtonKind)
                                        }
                                    >
                                        <option value="NONE">Nessuno</option>
                                    ...
                                    <div className="md:col-span-2">
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <Button
                                                ...
                                                disabled={form.quickReplies.length >= 3}
                                            >
                                                + Aggiungi
                                            </Button>
                                        </div>
                                        ...
                                        {errors.quickReplies && (
                                            <ErrorMsg>{errors.quickReplies}</ErrorMsg>
                                        )}
                                    </div>
                                )}
                                    ...
                                    <div className="md:col-span-2 space-y-3">
                                        ...
                                        {form.ctas.map((c, i) => (
                                            ...
                                            >
                                                ...
                                                {errors[`cta_${i}`] && (
                                                    <ErrorMsg>{errors[`cta_${i}`]}</ErrorMsg>
                                                )}
                                                {errors[`cta_${i}_url`] && (
                                                    <ErrorMsg>
                                                        {errors[`cta_${i}_url`]}
                    ...
                    {/* COLONNA DESTRA: PREVIEW + AZIONI */}
                    <div className="space-y-4 lg:sticky lg:top-24 self-start">
                        <Card title="Anteprima WhatsApp">
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        ...
                                        if (Object.keys(validation).length === 0) {
                                            setIsDialogApprovazioneOpen(true);
                                        }
                                    }}
                                >
                                    Invia per approvazione (Meta)
                                </Button>
                                {Object.keys(errors).length > 0 && (
...
export default withSessionCheck(NuovaVersioneTemplatesWhatsappSuperAdmin);
...
const WhatsAppView: React.FC<{ data: WhatsAppPreviewData }> = ({ data }) => {
    ...
    const bodyRendered = useMemo(
        () => renderWithExamples(bodyText || "", bodyExamples),
    ...
    return (
        <>
            ...
            >
                <div className="wa-message" style={{ justifyContent: "flex-end" }}>
                    <div className="wa-bubble">
                        ...
                        {headerFormat === "TEXT" && headerText?.trim() && (
                            <div className="wa-header-text">{headerText}</div>
                        )}
                        ...
                        <div className="wa-content">
                            ...
                            {footerText && (
                                <div className="wa-footer-text">{footerText}</div>
                            )}
                        </div>
                                    ...
                                    <div className="wa-cta-buttons">
                                        {ctaButtons.map((c, idx) => (
                                            <button
                                                key={idx}
                                                type="button"
                                                className={`wa-cta-button wa-cta-${c.type === "URL" ? "url" : "phone"
                                                    }`}
                                                title={
                                                    c.type === "URL"
                                                        ? c.url || ""
                ...
                {(language || category) && (
                    <div
                        className="wa-info-badge"
                        style={{ position: "static", marginTop: 12, color: "#e9edef" }}
                    >
...
type WhatsAppMessageShadowPreviewProps = {
...
const WhatsAppMessageShadowPreview: React.FC<WhatsAppMessageShadowPreviewProps> = ({
    data,
    hostStyle,
    isSubmitting,
    error,
    ...
    const previewContent = (
        <>
            {isSubmitting && (
                <div style={{ textAlign: "center", color: "#53bdeb", marginBottom: 8 }}>
                    Invio in corso...
                </div>
            )}
            {error && (
                <div style={{ textAlign: "center", color: "#e53935", marginBottom: 8 }}>
                    {error}
                </div>
            )}
            <WhatsAppView data={data} />
        </>
    );

    return (
        <div ref={wrapperRef} style={{ display: "inline-block", width: "100%" }}>
            {shadowContainer && createPortal(previewContent, shadowContainer)}
        </div>
    );
};
