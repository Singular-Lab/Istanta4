...
describe("coopfi policy v2 fixture", () => {
  it("builds expected tree for multi-settore + area", () => {
    ...
    expect(result.tree[0].children[0].children[0].nodeType).toBe("Reparto");
    expect(result.tree[0].children[0].children[0].nodeValue).toBe("CAPE");
    expect(result.tree[0].children[0].children[0].children[0].nodeType).toBe("Area");
    expect(result.tree[0].children[0].children[0].children[0].nodeValue).toBe("TDM");
  });
